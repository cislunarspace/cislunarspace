...
### 观测原理
热像素是传感器硅晶格中因制造缺陷或辐射损伤形成的载流子异常生成中心，在无光照条件下仍产生高于正常水平的暗电流信号。其在图像中表现为位置固定的异常亮像素，与真实天体源在各帧中的位置变化特征形成对比。

### 算法流程
去除流程为：对每个像素取其周围 3×3 邻域的中值作为替换值 → 热像素因异常高的值被中值替换为正常背景水平 → 真实天体源因跨越多个像素而保留大部分信号。该操作安排在叠加搜索算法之前，确保送入 SAA 的图像已清除热像素干扰。

### 精度分析
3×3 中值滤波在去除单个热像素的同时可能产生微小亮度异常，经过多次图像处理后可能演变为可检测的伪源。在宽视场传感器中热像素数量可观，若不妥善处理将严重影响虚警率；SAA 算法对不同速度假设下的图像进行平移叠加，热像素可能与真实移动目标轨迹重合，进一步放大虚警问题。

## 应用价值
热像素处理是地月空间移动天体光学巡天流水线中不可忽视的环节，直接影响叠加搜索算法的虚警率和检测效率。有效的热像素去除可显著降低后续处理阶段的计算负担，提高巡天系统对暗弱移动目标的检测可靠性。
