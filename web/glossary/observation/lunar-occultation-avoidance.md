---
title: 月掩规避（Lunar Occultation Avoidance）
description: 设计平动点轨道时，避免航天器被月球遮挡而与地球失去通信可视性的约束条件。在地月系L2点附近选取halo或Lissajous轨道时，需确保全弧段不发生月球对地通信链路的遮挡，这直接影响轨道幅值和构型的选择。
keywords: 月掩规避, Lunar Occultation Avoidance, 观测, 传感器, 可见性
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 月掩规避（Lunar Occultation Avoidance）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 月掩规避详解 | 术语定义
  description: 设计平动点轨道时，避免航天器被月球遮挡而与地球失去通信可视性的约束条件。在地月系L2点附近选取halo或Lissajous轨道时，需确保全弧段不发生月球对地通信链路的遮挡，这直接影响轨道幅值和构型的选择。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 月掩规避详解 | 术语定义
  description: 设计平动点轨道时，避免航天器被月球遮挡而与地球失去通信可视性的约束条件。在地月系L2点附近选取halo或Lissajous轨道时，需确保全弧段不发生月球对地通信链路的遮挡，这直接影响轨道幅值和构型的选择。
  image: /logo.png
permalink: /glossary/observation/lunar-occultation-avoidance/
---

# 月掩规避（Lunar Occultation Avoidance）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
>本站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

设计平动点轨道时，避免航天器被月球遮挡而与地球失去通信可视性的约束条件。在地月系L2点附近选取halo或Lissajous轨道时，需确保全弧段不发生月球对地通信链路的遮挡，这直接影响轨道幅值和构型的选择。

## 应用价值

月掩规避在地月空间态势感知中有重要应用，可用于目标检测、跟踪和识别。 该方法适用于月球轨道器和地月空间平台的观测任务设计。 利用该技术可以提高对空间目标的观测覆盖率和探测灵敏度。

## 相关概念

- [星间测距（Inter-Satellite Link Ranging）](/glossary/navigation/inter-satellite-ranging/)
- 目标覆盖时间占比（Target Coverage Time Ratio）
- [月掩规避（Lunar Occultation Avoidance）](/glossary/observation/lunar-occultation-avoidance/)
- [月球排除角（Lunar Exclusion Angle）](/glossary/observation/lea/)

## 参考文献

- 梁伟光 等, 2017, 深空探测学报, 4(2): 166-170
