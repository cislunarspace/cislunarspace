...
背景恒星消除利用 SExtractor 源检测工具生成的分割图（Segmentation Map）作为掩模，将图像中所有属于恒星的像素标记并置零。其核心思想是：恒星信号在各帧中位置固定，而移动目标信号位置变化，消除恒星信号可避免其干扰叠加搜索算法。

### 算法流程

处理流程为：使用 SExtractor 对配准后的图像进行源检测 → 生成分割图（非零像素表示恒星区域）→ 将分割图中所有非零像素标记为掩模 → 将被掩模的像素值设为零 → 输出无恒星干扰的图像供后续 SAA 处理。

### 精度分析

消除效果高度依赖检测阈值选择：高阈值可能导致恒星边缘像素未被标记，产生残留信号干扰；低阈值可能误将暗弱移动目标消除。需在充分移除恒星信号与保留目标信号完整性之间取得平衡，尤其在密集星场和月球眩光区内更为关键。

## 应用价值

背景恒星消除是地月空间移动天体光学巡天流水线中承上启下的关键步骤，直接影响叠加搜索算法的效率和检测灵敏度。在密集星场和月球散射光环境下，该技术的有效性决定了巡天系统对暗弱移动目标的探测能力。
