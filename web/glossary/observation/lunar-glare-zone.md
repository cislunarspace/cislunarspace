...
### 观测原理
月球眩光区的成因是月面反射太阳光在光学系统内部产生杂散光，淹没来自暗弱空间目标的信号。月球反照率约 0.12，但因距地球近且视面积大，其天空亮度极高。眩光影响范围取决于月心距：15° 以内信噪比严重下降，2° 以内传统上认为完全不可观测。

### 算法流程
在眩光区内观测的处理流程为：采集月球附近的短曝光图像 → 背景估计与扣除（需考虑月面散射光的非均匀分布）→ 先进的目标提取算法在低 SNR 条件下检测候选源 → 结合移位叠加技术增强移动目标信号 → 通过星历关联确认目标身份。

### 精度分析
眩光区内观测精度受月相、望远镜光学设计、大气条件和探测器灵敏度共同制约。满月时眩光最强影响范围最大；先进的背景扣除和目标提取算法可在一定程度上缓解眩光影响，但信噪比仍显著低于远离月球区域的观测。

## 应用价值
月球眩光区的有效观测对地月空间态势感知具有重要意义：许多重要轨道（如 NRHO 的月球近端弧段）穿越该区域。将有效观测边界从传统的 15° 向内压缩可显著扩大光学巡天的可观测区域，提升空间态势感知的完整性。
