---
title: 混合通信导航星座（Hybrid Communication/Navigation Constellation）
description: 同时集成精密授时定位与高速数据中继功能的月球综合空间基础设施星座。
keywords: 混合通信导航星座, 通导一体化, 空间星座, 星间链路
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 混合通信导航星座
  desc: 通信导航一体化月球空间星座设计。
  image: /logo.png
permalink: /glossary/communication/hybrid-communication-navigation-constellation/
---

# 混合通信导航星座（Hybrid Communication/Navigation Constellation）

## 定义

通过共享星载载荷与星间测距链路，在地月空间同时提供自主几何定位、高精度时钟同步与宽带数据传输的一体化卫星星座系统。

## 物理机制与工程价值

传统深空任务依赖地球地面测控网进行双向测距测速，存在视线盲区与天线资源竞争瓶颈。混合星座将导航测距信号调制在通信载波上，各卫星节点通过星间双向测距建立自主时间基准与几何基线，为月面移动终端及环月飞行器提供四星几何交会定轨服务，同时作为高带宽中继节点将科学数据回传地球。

## 相关概念

- [天基自主定轨](/glossary/navigation/aod/)
- [星间通视](/glossary/communication/inter-satellite-visibility/)
- [异构中继星座](/glossary/communication/heterogeneous-relay-constellation/)

## 参考文献

- Gao and Hou, 2020, Cislunar constellation design for lunar navigation and communication.
