...
CR3BP-LT 是标准圆形限制性三体问题（Circular Restricted Three-Body Problem, CR3BP）的扩展，通过在飞行器的运动方程中加入连续低推力加速度项，用以研究电推进、离子推进等低推力推进系统的轨道转移问题。
...
$$
\begin{cases}
\ddot{x} = -\left[(1-\mu)\dfrac{x+\mu}{r_1^3} + \mu\dfrac{x-(1-\mu)}{r_2^3}\right] + 2\dot{y} + x + \dfrac{\tilde{T}_{\max}}{\tilde{m}} u_x \\[0.8em]
\ddot{y} = -\left[(1-\mu)\dfrac{y}{r_1^3} + \mu\dfrac{y}{r_2^3}\right] - 2\dot{x} + y + \dfrac{\tilde{T}_{\max}}{\tilde{m}} u_y \\[0.8em]
\ddot{z} = -\left[(1-\mu)\dfrac{z}{r_1^3} + \mu\dfrac{z}{r_2^3}\right] + \dfrac{\tilde{T}_{\max}}{\tilde{m}} u_z
\end{cases}
$$

其中 $r_1 = \sqrt{(x+\mu)^2 + y^2 + z^2}$、$r_2 = \sqrt{(x-1+\mu)^2 + y^2 + z^2}$ 分别为飞行器到地球和月球的距离，$-2\dot{y}$、$+2\dot{x}$ 为 Coriolis 力项，$u = (u_x, u_y, u_z)$ 为无量纲推力控制向量，满足 $\|u\| \leq 1$。

质量变化服从齐奥尔科夫斯基火箭方程：

$$
\dot{\tilde{m}} = -\dfrac{\tilde{T}_{\max}}{\tilde{c}} \|u\|
$$
...
[2] Du C, Song L, Zhang J, et al. A novel calculation method for low-thrust transfer trajectories in the Earth-Moon restricted three-body problem[J]. Aerospace Science and Technology, 2024, 147: 109048.
