...
作用角变量由共轭变量 $(I, \theta)$ 组成，作用量 $I = \frac{1}{2\pi}\oint p \, dq$ 沿闭合轨道一周积分，角变量 $\theta$ 描述轨道在环面上的转动相位。在可积系统中，哈密顿量仅依赖于作用量 $H = H(I)$。

### 关键性质

$I$ 描述不变环面的截面半径，$\theta$ 描述轨道在环面上的转动相位。在 CR3BP 共线平动点的线性化动力学中，中心模的作用角变量为 $I_c = \frac{1}{2}(q^2 + p^2)$、$\theta_c = \arctan(q/p)$。
