---
title: Weierstrass-Erdmann角点条件（Weierstrass-Erdmann Corner Conditions）
description: 变分学中描述最优轨迹在控制突变点（角点）处伴随变量和哈密顿函数连续性的条件。
keywords: Weierstrass-Erdmann角点条件, Weierstrass-Erdmann Corner Conditions, 轨道力学, 动力学, 控制
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: Weierstrass-Erdmann角点条件（Weierstrass-Erdmann Corner Conditions）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: Weierstrass-Erdmann角点条件详解 | 术语定义
  description: 变分学中描述最优轨迹在控制突变点（角点）处伴随变量和哈密顿函数连续性的条件。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: Weierstrass-Erdmann角点条件详解 | 术语定义
  description: 变分学中描述最优轨迹在控制突变点（角点）处伴随变量和哈密顿函数连续性的条件。
  image: /logo.png
permalink: /glossary/dynamics/weierstrass-erdmann-corner-conditions/
---

# Weierstrass-Erdmann角点条件（Weierstrass-Erdmann Corner Conditions）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 本文地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

变分学中描述最优轨迹在控制突变点（角点）处伴随变量和哈密顿函数连续性的条件。

## 应用价值

该概念在地月空间研究和工程实践中具有应用价值，有助于理解该领域的核心原理。

## 相关概念

- [蒙特卡洛轨迹打靶法（Monte Carlo Trajectory Shooting, MCTS）](/glossary/dynamics/mcts/)
- 有动力月球借力（Powered Lunar Flyby, PLF）
- [平面圆形限制性三体问题（Planar Circular Restricted Three-Body Problem, PCRTBP）](/glossary/dynamics/cr3bp/)
- 插入速度增量（Insertion Delta-V）

## 参考文献

- Grossi 等 - 2024。
