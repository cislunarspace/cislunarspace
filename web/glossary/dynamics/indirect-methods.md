...
description: 轨迹优化的一类方法，通过变分法和庞特里亚金极值原理推导一阶必要条件（协态方程、哈密顿函数等），再通过打靶法求解两点边值问题。间接法精度高、收敛阶数高，但对初始猜测敏感，协态初值难以选取。
keywords: 间接法, Indirect Methods, 轨道优化, 控制理论, 非线性控制
author: 天疆说
date: 2026-07-31
...
轨迹优化的一类方法，通过变分法和庞特里亚金极值原理推导一阶必要条件（协态方程、哈密顿函数等），再通过打靶法求解两点边值问题。间接法精度高、收敛阶数高，但对初始猜测敏感，协态初值难以选取。
