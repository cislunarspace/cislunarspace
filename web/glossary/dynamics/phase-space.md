...
其几何实质是：在某一能量水平下，零速度面尚未完全封闭的区域边界处，不变流形（特别是 $L_1$ 或 $L_2$ 点的稳定/不稳定流形管）穿越原本看似封闭的禁入区，形成一个狭窄的相空间通道。航天器若恰好在通道的流形上（或通过小量冲量进入），即可在两个区域之间实现"自然的"低能量转移。这是基于不变流形的弱稳定边界（WSB）转移方法的核心动力学原理（Koon et al. 2011）。
