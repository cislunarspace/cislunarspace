---
title: 角速度（Angular Rate）
description: 位置矢量相对于中心天体扫过角度的时间变化率。在形状基方法的推导中，角速度由形状函数通过二阶多项式方程解析确定，是计算推力加速度和飞行时间的关键中间量。
keywords: 角速度, Angular Rate, 轨道, 动力学, 控制
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 角速度（Angular Rate）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 角速度详解 | 术语定义
  description: 位置矢量相对于中心天体扫过角度的时间变化率。在形状基方法的推导中，角速度由形状函数通过二阶多项式方程解析确定，是计算推力加速度和飞行时间的关键中间量。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 角速度详解 | 术语定义
  description: 位置矢量相对于中心天体扫过角度的时间变化率。在形状基方法的推导中，角速度由形状函数通过二阶多项式方程解析确定，是计算推力加速度和飞行时间的关键中间量。
  image: /logo.png
permalink: /glossary/dynamics/angular-rate/
---

# 角速度（Angular Rate）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
>本站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

位置矢量相对于中心天体扫过角度的时间变化率。在形状基方法的推导中，角速度由形状函数通过二阶多项式方程解析确定，是计算推力加速度和飞行时间的关键中间量。

## 应用价值

该概念在地月空间研究和任务设计中具有基础性支撑作用。在实际任务中，需要根据具体应用场景和约束条件选择合适的分析方法。。

## 相关概念

- [保守系统（Conservative System）](/glossary/dynamics/conservative-system/)
- [空间流形动力学（Space Manifold Dynamics, SMD）](/glossary/dynamics/space-manifold-dynamics-smd/)
- [RSW坐标系（Radial-Transverse-Normal Frame, RSW Frame）](/glossary/dynamics/radial-transverse-normal-frame-rsw-frame/)

## 参考文献

- Vellutini & Avanzini, 2014, Shape-based design of low-thrust trajectories to cislunar lagrangian point
