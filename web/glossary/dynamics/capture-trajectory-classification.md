---
title: 捕获轨道行为分类（Capture Trajectory Classification）
description: 在庞加莱截面网格扫描法中，根据轨道在月球引力范围内的行为将初始条件分为五类：良好捕获（G）、低轨道捕获（L）、高轨道捕获（H）、撞月（C）和逃逸（O）。该分类为后续优化提供精准的候选解集。
keywords: 捕获轨道行为分类, Capture Trajectory Classification, 动力学, 摄动, 轨道
author: 天疆说
date: 2026-07-31
...
twitter:
  ...
  description: 在庞加莱截面网格扫描法中，根据轨道在月球引力范围内的行为将初始条件分为五类：良好捕获（G）、低轨道捕获（L）、高轨道捕获（H）、撞月（C）和逃逸（O）。该分类为后续优化提供精准的候选解集。
  image: /logo.png
permalink: /glossary/dynamics/capture-trajectory-classification/
---

# 捕获轨道行为分类（Capture Trajectory Classification）
...
在庞加莱截面网格扫描法中，根据轨道在月球引力范围内的行为将初始条件分为五类：良好捕获（G）、低轨道捕获（L）、高轨道捕获（H）、撞月（C）和逃逸（O）。该分类为后续优化提供精准的候选解集。
