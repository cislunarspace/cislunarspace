---
title: 时间延迟坐标（Time-Delay Coordinates）
description: 从时间序列重构动力系统吸引子的方法。将标量时间序列{x(t), x(t+T), ..., x(t+(d-1)T)}嵌入d维空间，可恢复原系统几何结构。
...
twitter:
  ...
  description: 从时间序列重构动力系统吸引子的方法。将标量时间序列{x(t), x(t+T), ..., x(t+(d-1)T)}嵌入d维空间，可恢复原系统几何结构。
  image: /logo.png
permalink: /glossary/dynamics/time-delay-coordinates/
---

# 时间延迟坐标（Time-Delay Coordinates）
...
从时间序列重构动力系统吸引子的方法。将标量时间序列{x(t), x(t+T), ..., x(t+(d-1)T)}嵌入d维空间，可恢复原系统几何结构。
