---
title: Runge效应（Runge Effect）
description: 在等距节点上使用高次多项式插值时，区间两端出现剧烈振荡的现象。采用CGL点等非等距节点分布可以抑制此效应。
keywords: Runge效应, Runge Effect, 动力学, 摄动, 轨道
author: 天疆说
date: 2026-07-31
...
twitter:
  ...
  description: 在等距节点上使用高次多项式插值时，区间两端出现剧烈振荡的现象。采用CGL点等非等距节点分布可以抑制此效应。
  image: /logo.png
permalink: /glossary/dynamics/runge-effect/
---

# Runge效应（Runge Effect）
...
在等距节点上使用高次多项式插值时，区间两端出现剧烈振荡的现象。采用CGL点等非等距节点分布可以抑制此效应。
