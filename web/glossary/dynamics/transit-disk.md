---
title: 穿越盘（Transit Disk）
description: 平衡区域L的边界二维球面上，由穿越轨道组成的两个开圆盘。一个圆盘中的轨道从左向右穿越，另一个从右向左穿越。穿越盘的边界由渐近轨道（Moser定理下的解析圆）组成。这个拓扑结构是Conley将线性化结果推广到非线性情形的关键中间步骤。
keywords: 穿越盘, Transit Disk, 轨道动力学, 多体问题, 摄动
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 穿越盘（Transit Disk）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 穿越盘详解 | 术语定义
  description: 平衡区域L的边界二维球面上，由穿越轨道组成的两个开圆盘。一个圆盘中的轨道从左向右穿越，另一个从右向左穿越。穿越盘的边界由渐近轨道（Moser定理下的解析圆）组成。这个拓扑结构是Conley将线性化结果推广到非线性情形的关键中间步骤。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 穿越盘详解 | 术语定义
  description: 平衡区域L的边界二维球面上，由穿越轨道组成的两个开圆盘。一个圆盘中的轨道从左向右穿越，另一个从右向左穿越。穿越盘的边界由渐近轨道（Moser定理下的解析圆）组成。这个拓扑结构是Conley将线性化结果推广到非线性情形的关键中间步骤。
  image: /logo.png
permalink: /glossary/dynamics/transit-disk/
---

# 穿越盘（Transit Disk）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
>本站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

平衡区域L的边界二维球面上，由穿越轨道组成的两个开圆盘。一个圆盘中的轨道从左向右穿越，另一个从右向左穿越。穿越盘的边界由渐近轨道（Moser定理下的解析圆）组成。这个拓扑结构是Conley将线性化结果推广到非线性情形的关键中间步骤。

## 应用价值

在轨道设计与优化中，该概念用于分析航天器在地月空间多体引力场中的运动特性，为低能量转移轨道设计提供理论依据。

## 相关概念

- [Lorillo稳定性准则（Lorillo Stability Criterion）](/glossary/dynamics/lorillo-stability-criterion/)
- [稳定特征向量（Stable Eigenvector）](/glossary/dynamics/stable-eigenvector/)
- [月球飞越法（Lunar Fly-by Method）](/glossary/dynamics/lunar-fly-by-method/)
- [可达集（Reachability Set）](/glossary/dynamics/reachability-set/)

## 参考文献

- Conley - 1968 - Low energy transit orbits in restricted 3-body problem
