...
$\vec\omega$ 是 $\mathcal R$ 相对 $\mathcal I$ 的瞬时角速度。矢量本身与参考系无关，但其分量与"对时间求导"的操作随参考系而变——科氏定理正是后者的变换法则。它是 [会合坐标系](/glossary/fundamentals/synodic-frame/)、月固系、地球固系等所有旋转参考系中航天器运动方程的推导根基。
...
把 $\Omega$ 写成"两体引力势 + 离心势"的统一标量函数后，运动方程右端的科氏项 $-2\vec\omega\times\dot{\vec r}$ 是**陀螺力**（不做功、垂直于速度），由此可推出 [雅可比积分](/glossary/dynamics/jacobi-integral/) 守恒。这就是会合系方程中"科氏 + 离心"两项的力学根源。
