...
### 数学定义
B-G 标准型通过 Legendre 展开将 CR3BP 哈密顿量在平动点邻域展开为齐次多项式序列 $H = \sum_{n \geq 2} H_n$，再通过李变换逐步消除非共振项，得到对角化的标准型。

### 关键性质
线性项 $H_2$ 具有鞍×中心×中心结构。展开阶数 $N$ 超过 13 时误差减小趋缓（受限于双精度浮点精度），建议取 $N=15$。

### 数值方法
通过逐阶李变换生成函数 $G_n$，对 $n$ 阶项执行 $\hat{H}_n = H_n + \{H_2, G_n\}$，选择适当的生成函数消除非共振项并保留共振项。

## 应用价值

B-G 标准型是平动点轨道族参数化和相空间结构分析的基础工具，与中心流形理论结合后可将六维相空间分解为双曲方向和中心方向，建立从笛卡尔坐标到特征参数的双射对应，为轨道辨识和分类提供理论基础。
