---
title: 后向调相（Receding Phasing）
description: 时间相位差为负值的调相方式，即调相轨道到达目标点的时间晚于目标航天器自然运动到该点的时间。在DRO调相中，后向调相轨道运行路径位于DRO外侧，可实现的调相范围通常大于前向调相。
keywords: 后向调相, Receding Phasing, 轨道动力学, 姿态控制, 稳定性
author: 天疆说
date: 2026-07-31
...
twitter:
  ...
  description: 时间相位差为负值的调相方式，即调相轨道到达目标点的时间晚于目标航天器自然运动到该点的时间。在DRO调相中，后向调相轨道运行路径位于DRO外侧，可实现的调相范围通常大于前向调相。
  image: /logo.png
permalink: /glossary/dynamics/receding-phasing/
---

# 后向调相（Receding Phasing）
...
时间相位差为负值的调相方式，即调相轨道到达目标点的时间晚于目标航天器自然运动到该点的时间。在DRO调相中，后向调相轨道运行路径位于DRO外侧，可实现的调相范围通常大于前向调相。
