---
title: 逃逸运动（Escape Motion）
description: 三体系统中至少一个天体趋向无穷远的运动，逃逸条件由能量和雅可比积分等决定。
keywords: 逃逸运动, Escape Motion, 轨道动力学, 轨道优化, 非线性动力学
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 逃逸运动（Escape Motion）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 逃逸运动详解 | 术语定义
  description: 三体系统中至少一个天体趋向无穷远的运动，逃逸条件由能量和雅可比积分等决定。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 逃逸运动详解 | 术语定义
  description: 三体系统中至少一个天体趋向无穷远的运动，逃逸条件由能量和雅可比积分等决定。
  image: /logo.png
permalink: /glossary/dynamics/escape-motion/
---

# 逃逸运动（Escape Motion）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
>本站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

三体系统中至少一个天体趋向无穷远的运动，逃逸条件由能量和雅可比积分等决定。

## 应用价值

该模型是分析地月空间动力学、设计低能量转移轨道的基础工具。该矩阵在可观测性分析和滤波器设计中起关键作用。分析逃逸条件有助于评估物体是否会离开目标天体附近区域。

## 相关概念

- 希尔球半径（Hill Sphere Radius）
- [伪谱凸优化（Pseudospectral Convex Optimization）](/glossary/dynamics/pseudospectral-convex-optimization/)
- [庞加莱映射表示（Poincaré Map Representation）](/glossary/dynamics/poincar-map-representation/)
- [最小范数靶点法（Minimum Norm Targeting）](/glossary/dynamics/minimum-norm-targeting/)

## 参考文献

- Marchal - 1990 - The three-body problem
