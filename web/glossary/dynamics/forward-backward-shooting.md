---
title: 前后向打靶法（Forward-Backward Shooting）
description: 将转移轨道分为前后两段分别向前和向后积分，通过匹配中间位置来求解双Lambert问题的数值方法。
keywords: 前后向打靶法, Forward-Backward Shooting, 轨道动力学, 姿态控制, 稳定性
author: 天疆说
date: 2026-07-31
...
twitter:
  ...
  description: 将转移轨道分为前后两段分别向前和向后积分，通过匹配中间位置来求解双Lambert问题的数值方法。
  image: /logo.png
permalink: /glossary/dynamics/forward-backward-shooting/
---

# 前后向打靶法（Forward-Backward Shooting）
...
将转移轨道分为前后两段分别向前和向后积分，通过匹配中间位置来求解双Lambert问题的数值方法。
