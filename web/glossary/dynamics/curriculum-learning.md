...
### 数学定义
课程学习定义为任务难度递增的序列 $C = \{(N_i, \Delta d_i, \Delta v_i)\}$，其中 $N_i$ 为训练步数阈值，$\Delta d_i$ 和 $\Delta v_i$ 为对应的成功容忍度。

### 关键性质
课程学习通过避免稀疏奖励陷阱、稳定梯度估计和避免局部最优来提升训练效果。从简单任务起步允许 Agent 探索更大的状态空间。

### 数值方法
课程调度采用基于全局步数的分段策略，每次阈值收紧时训练曲线呈现特征性的"台阶"形态，Agent 适应后恢复稳定。

## 应用价值

课程学习在高难度、长 horizon 的地月空间低推力轨迹优化中至关重要，使 Agent 在混沌动力学环境中逐步建立对问题的理解，实现从粗略到达精确轨道插入的渐进式学习。
