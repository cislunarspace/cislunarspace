...
其中 $\mu=m_2/(m_1+m_2)$ 为质量参数，$r_1$、$r_2$ 为到两主天体的距离，$v$ 为航天器在会合系中的速度大小（Vallado 2022, Eq. 12-15；Szebehely 1967, §1.6）。Vallado 称其为伪积分，并非因为推导可疑，而是因为它只存在于会合系这个特殊参考系下、且仅对限制性三体问题成立。
...
- **有效势 / 等效势**（effective potential）：最通用叫法，强调把离心项并入势后方程可写成 $\ddot{\mathbf r}+2\hat{\mathbf z}\times\dot{\mathbf r}=\nabla\Omega$ 的紧凑形式；
...
C 是航天器单凭动力学（无 $\Delta v$）能到达哪片区域的第一判据：从地球附近出发要够得着月球，必须把 C 降到 $C_1$ 以下；要去 $L_2$ 以外的深空，必须降到 $C_2$ 以下。低能地月转移的设计本质就是在合适的时机、以最小的 $\Delta v$ 把 C 从近地停泊轨道的值（约 $3.44$，地球引力主导）降到 $C_1\approx 3.188$ 以下。
