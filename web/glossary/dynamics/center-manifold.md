...
**NHIM**（Normally Hyperbolic Invariant Manifold，法向双曲不变流形）是中心流形概念的推广：它本身可以是多维不变流形，沿其法向具有强烈的双曲性（稳定/不稳定方向指数收缩/扩张），而切向则表现为中性或弱双曲。在 CR3BP 中，平动点周期轨道本身是一维 NHIM：切向沿轨道中性，法向有稳定、不稳定及中心方向。NHIM 是相空间中控制输运的瓶颈结构，也是构造异宿循环和不变环面的基础（Wiggins 1994；Ross 2022）。
