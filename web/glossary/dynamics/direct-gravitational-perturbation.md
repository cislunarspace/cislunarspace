---
title: 直接引力（Direct Gravitational Perturbation）
description: 第三天体（如太阳）对探测器的直接引力作用。在地月平动点导航动力学模型中，太阳对探测器的引力加速度与太阳对地月质心的引力加速度之差构成太阳的直接引力摄动项。
keywords: 直接引力, Direct Gravitational Perturbation, , 轨道动力学, 三体问题, 平动点
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 直接引力（Direct Gravitational Perturbation）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 直接引力详解 | 术语定义
  description: 第三天体（如太阳）对探测器的直接引力作用。在地月平动点导航动力学模型中，太阳对探测器的引力加速度与太阳对地月质心的引力加速度之差构成太阳的直接引力摄动项。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 直接引力详解 | 术语定义
  description: 第三天体（如太阳）对探测器的直接引力作用。在地月平动点导航动力学模型中，太阳对探测器的引力加速度与太阳对地月质心的引力加速度之差构成太阳的直接引力摄动项。
  image: /logo.png
permalink: /glossary/dynamics/direct-gravitational-perturbation/
---

# 直接引力（Direct Gravitational Perturbation）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 本文来源：[地月空间入门指南](https://cislunarspace.cn)

## 定义

第三天体（如太阳）对探测器的直接引力作用。在地月平动点导航动力学模型中，太阳对探测器的引力加速度与太阳对地月质心的引力加速度之差构成太阳的直接引力摄动项。

## 应用价值

直接引力是第三体对航天器的直接引力加速度，是三体系统中主要的摄动源之一，轨道设计师计算轨道时必须考虑直接引力摄动。

## 相关概念

- [渐近解（Asymptotic Solution）](/glossary/dynamics/asymptotic-solution/)
- [近月点数据库（Perilune Database）](/glossary/dynamics/perilune-database/)
- [平动点轨道编目（Libration Point Orbit Cataloging）](/glossary/orbits/libration-point-periodic-orbit/)
- [Floquet模态法（Floquet Modal Method）](/glossary/dynamics/floquet-modal-method/)

## 参考文献

- 钱霙婧等, 2013, 宇航学报
