---
title: 切向注入（Tangential Injection）
description: 内部转移中的一种注入方式，施加的脉冲方向与飞行器位置矢量垂直（即沿速度方向），使飞行器沿切线方向进入渐近轨道。优点是转移时间较短，但燃料消耗相对较多。
keywords: 切向注入, Tangential Injection, 轨道力学, 姿态控制, 相对运动
author: 天疆说
date: 2026-07-31
...
twitter:
  ...
  description: 内部转移中的一种注入方式，施加的脉冲方向与飞行器位置矢量垂直（即沿速度方向），使飞行器沿切线方向进入渐近轨道。优点是转移时间较短，但燃料消耗相对较多。
  image: /logo.png
permalink: /glossary/dynamics/tangential-injection/
---

# 切向注入（Tangential Injection）
...
内部转移中的一种注入方式，施加的脉冲方向与飞行器位置矢量垂直（即沿速度方向），使飞行器沿切线方向进入渐近轨道。优点是转移时间较短，但燃料消耗相对较多。
