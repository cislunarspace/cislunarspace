---
title: 入轨方向（Orbital Insertion Direction）
description: 航天器进入目标轨道时相对于轨道运动的方向。加速入轨使航天器从目标轨道内侧插入，减速入轨从外侧插入。入轨方向与绕月方向的组合将DRO转移轨道分为逆行加速、逆行减速、顺行加速和顺行减速四类。
keywords: 入轨方向, Orbital Insertion Direction, 轨道动力学, 姿态控制, 稳定性
author: 天疆说
date: 2026-07-31
...
twitter:
  ...
  description: 航天器进入目标轨道时相对于轨道运动的方向。加速入轨使航天器从目标轨道内侧插入，减速入轨从外侧插入。入轨方向与绕月方向的组合将DRO转移轨道分为逆行加速、逆行减速、顺行加速和顺行减速四类。
  image: /logo.png
permalink: /glossary/dynamics/orbital-insertion-direction/
---

# 入轨方向（Orbital Insertion Direction）
...
航天器进入目标轨道时相对于轨道运动的方向。加速入轨使航天器从目标轨道内侧插入，减速入轨从外侧插入。入轨方向与绕月方向的组合将DRO转移轨道分为逆行加速、逆行减速、顺行加速和顺行减速四类。
