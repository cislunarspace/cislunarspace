---
title: 间接启发式方法（Indirect Heuristic Method）
description: 结合最优性必要条件（庞特里亚金极小值原理）与启发式算法（如差分进化）求解小推力轨道转移的数值方法。
keywords: 间接启发式方法, Indirect Heuristic Method, 轨道动力学, 轨道优化, 非线性动力学
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 间接启发式方法（Indirect Heuristic Method）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 间接启发式方法详解 | 术语定义
  description: 结合最优性必要条件（庞特里亚金极小值原理）与启发式算法（如差分进化）求解小推力轨道转移的数值方法。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 间接启发式方法详解 | 术语定义
  description: 结合最优性必要条件（庞特里亚金极小值原理）与启发式算法（如差分进化）求解小推力轨道转移的数值方法。
  image: /logo.png
permalink: /glossary/dynamics/indirect-heuristic-method/
---

# 间接启发式方法（Indirect Heuristic Method）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
>本站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

结合最优性必要条件（庞特里亚金极小值原理）与启发式算法（如差分进化）求解小推力轨道转移的数值方法。

## 应用价值

在设计地月转移方案时，利用该轨道特性可降低任务总速度增量需求。对于大质量航天器的长时间转移，该推进方式可大幅降低推进剂消耗。

## 相关概念

- 希尔球半径（Hill Sphere Radius）
- [伪谱凸优化（Pseudospectral Convex Optimization）](/glossary/dynamics/pseudospectral-convex-optimization/)
- [庞加莱映射表示（Poincaré Map Representation）](/glossary/dynamics/poincare-section/)
- [最小范数靶点法（Minimum Norm Targeting）](/glossary/dynamics/differential-correction/)

## 参考文献

- Pozzi 等 - 2025
