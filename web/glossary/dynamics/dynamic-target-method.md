...
动态靶点法引入位置系数 $\gamma_i \in [0, 1]$ 表示靶点在 DRO 轨道上的相对位置，以单圈 DRO 轨道保持控制所需的燃料消耗为目标函数，通过差分进化算法寻找最优靶点位置。

### 关键性质

与传统靶点法的等间隔固定选取不同，动态靶点法针对每条 DRO 轨道自适应调整靶点位置，燃料消耗减少约一半，机动次数减少约 25%。

### 数值方法

设置 2 个靶点时轨道保持效果最佳。优化过程采用差分进化算法在连续空间中搜索最优的 $\gamma_i$ 值。

## 应用价值

动态靶点法为 DRO 轨道的长期低能耗保持控制提供了有效手段，通过优化靶点位置显著降低燃料消耗，在一年期拟周期 DRO 轨道保持控制中表现出优越的性能。
