---
title: 共振定理（Near Resonance Theorem）
description: 将刘维尔定理和哈特曼定理推广到近共振情况的理论，证明了中心流形的存在及其与稳定、不稳定流形的相互作用。
keywords: 共振定理, Near Resonance Theorem, , 轨道动力学, 三体问题, 平动点
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 共振定理（Near Resonance Theorem）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 共振定理详解 | 术语定义
  description: 将刘维尔定理和哈特曼定理推广到近共振情况的理论，证明了中心流形的存在及其与稳定、不稳定流形的相互作用。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 共振定理详解 | 术语定义
  description: 将刘维尔定理和哈特曼定理推广到近共振情况的理论，证明了中心流形的存在及其与稳定、不稳定流形的相互作用。
  image: /logo.png
permalink: /glossary/dynamics/near-resonance-theorem/
---

# 共振定理（Near Resonance Theorem）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 本文来源：[地月空间入门指南](https://cislunarspace.cn)

## 定义

将刘维尔定理和哈特曼定理推广到近共振情况的理论，证明了中心流形的存在及其与稳定、不稳定流形的相互作用。

## 应用价值

共振定理证明了中心流形的存在及其与稳定、不稳定流形的相互作用，是分析近共振动力学系统的基础，帮助设计师理解轨道如何在不同流形间转移。

## 相关概念

- [渐近解（Asymptotic Solution）](/glossary/dynamics/asymptotic-solution/)
- [近月点数据库（Perilune Database）](/glossary/dynamics/perilune-database/)
- [平动点轨道编目（Libration Point Orbit Cataloging）](/glossary/orbits/libration-point-periodic-orbit/)
- [Floquet模态法（Floquet Modal Method）](/glossary/dynamics/floquet-modal-method/)

## 参考文献

- Marchal - 1990 - The three-body problem
