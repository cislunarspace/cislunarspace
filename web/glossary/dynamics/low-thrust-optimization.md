---
title: 小推力优化（Low-Thrust Optimization）
description: 优化低推力轨迹的数值方法，包括间接法和直接法。
keywords: 小推力优化, Low-Thrust Optimization, 轨道力学, 最优控制, 轨迹优化
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 小推力优化（Low-Thrust Optimization）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 小推力优化详解 | 术语定义
  description: 优化低推力轨迹的数值方法，包括间接法和直接法。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 小推力优化详解 | 术语定义
  description: 优化低推力轨迹的数值方法，包括间接法和直接法。
  image: /logo.png
permalink: /glossary/dynamics/low-thrust-optimization/
---

# 小推力优化（Low-Thrust Optimization）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
>本站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

优化低推力轨迹的数值方法，包括间接法和直接法。

## 应用价值

该控制方法在地月空间任务中可用于航天器的姿态稳定、轨道保持和机动轨迹优化，提升任务执行的灵活性和可靠性。

## 相关概念

- [混合差分动态规划（Hybrid Differential Dynamic Programming）](/glossary/dynamics/hddp/)
- [形状法（Shape Method）](/glossary/dynamics/shape-method/)
- 差分动态规划（Differential Dynamic Programming, DDP）
- 二阶锥规划（Second-Order Cone Programming, SOCP）

## 参考文献

- Ross 等 - 2022 - Dynamical systems, the three-body problem, and space mission design
