...
### 数学定义
Poincaré 截面选取 $N-2$ 维低维截面，记录轨道反复穿越该截面的交点。对于 CR3BP 共线平动点的四维中心流形，截面为二维。

### 关键性质
截面上闭合曲线对应准周期轨道（环面），孤立离散点对应周期轨道，混沌散点对应混沌运动。Halo 轨道起源于 Lyapunov 轨道随能量升高而发生的分叉。

### 数值方法
常用 $\theta_2 = 0$ 或 $\theta_3 = \pi/2$ 截面，在截面上选取均匀网格点并积分，叠加不同能量层等值线得到包含所有轨道族信息的"地图"。

## 应用价值

Poincaré 截面是平动点轨道分布可视化和轨道辨识的核心工具，通过"查地图"方式实现轨道辨识，绕过了在混沌环境中直接数值积分的困难。
