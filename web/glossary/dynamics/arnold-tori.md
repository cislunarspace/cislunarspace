---
title: 阿诺德环面（Arnold Tori）
description: 近可积哈密顿系统中由不变环面组成的相空间结构，KAM定理保证了在非共振条件下部分环面的持久性。
keywords: 阿诺德环面, Arnold Tori, 轨道动力学, 轨道优化, 非线性动力学
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 阿诺德环面（Arnold Tori）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 阿诺德环面详解 | 术语定义
  description: 近可积哈密顿系统中由不变环面组成的相空间结构，KAM定理保证了在非共振条件下部分环面的持久性。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 阿诺德环面详解 | 术语定义
  description: 近可积哈密顿系统中由不变环面组成的相空间结构，KAM定理保证了在非共振条件下部分环面的持久性。
  image: /logo.png
permalink: /glossary/dynamics/arnold-tori/
---

# 阿诺德环面（Arnold Tori）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
>本站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

近可积哈密顿系统中由不变环面组成的相空间结构，KAM定理保证了在非共振条件下部分环面的持久性。

## 应用价值

共振轨道可用于设计长期稳定的任务轨道，减少轨道维持代价。

## 相关概念

- [希尔球半径（Hill Sphere Radius）](/glossary/dynamics/hill-sphere-radius/)
- [伪谱凸优化（Pseudospectral Convex Optimization）](/glossary/dynamics/pseudospectral-convex-optimization/)
- [庞加莱映射表示（Poincaré Map Representation）](/glossary/dynamics/poincar-map-representation/)
- [最小范数靶点法（Minimum Norm Targeting）](/glossary/dynamics/minimum-norm-targeting/)

## 参考文献

- Marchal - 1990 - The three-body problem
