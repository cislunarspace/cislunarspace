---
title: 脉冲宽度调制（Pulse-Width Modulation, PWM）
description: 将MPC输出的变心动量转换为固定推力发动机的实际点火时长的调制策略。根据推力器可用加速度将所需的速度增量转换为对应的工作时间，在每个采样间隔内先开机加速后滑行至下一采样点。
keywords: 脉冲宽度调制, Pulse-Width Modulation, PWM, PWM, 轨道动力学, 控制理论, 非线性控制, 最优控制
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 脉冲宽度调制（Pulse-Width Modulation, PWM）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 脉冲宽度调制详解 | 术语定义
  description: 将MPC输出的变心动量转换为固定推力发动机的实际点火时长的调制策略。根据推力器可用加速度将所需的速度增量转换为对应的工作时间，在每个采样间隔内先开机加速后滑行至下一采样点。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 脉冲宽度调制详解 | 术语定义
  description: 将MPC输出的变心动量转换为固定推力发动机的实际点火时长的调制策略。根据推力器可用加速度将所需的速度增量转换为对应的工作时间，在每个采样间隔内先开机加速后滑行至下一采样点。
  image: /logo.png
permalink: /glossary/dynamics/PWM/
---

# 脉冲宽度调制（Pulse-Width Modulation, PWM）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
>本站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

将MPC输出的变心动量转换为固定推力发动机的实际点火时长的调制策略。根据推力器可用加速度将所需的速度增量转换为对应的工作时间，在每个采样间隔内先开机加速后滑行至下一采样点。

## 应用价值

将变比冲输出转换为固定推力发动机实际点火时长的调制策略，实现对连续最优控制的近似执行。

## 相关概念

- [规定性能控制（Prescribed Performance Control, PPC）](/glossary/dynamics/PPC/)
- [规定性能函数（Prescribed Performance Function, PPF）](/glossary/dynamics/PPF/)
- [动力下降制导（Powered Descent Guidance）](/glossary/dynamics/PDG/)
- [平面双圆限制性四体问题（Planar Bicircular Restricted Four-Body Problem）](/glossary/dynamics/PBR4BP/)

## 参考文献

- Capannolo 等 - 2023 - Model predictive control for formation reconfiguration exploiting quasi-periodic tori in the cislunar environment。
