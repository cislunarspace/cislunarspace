---
title: 月球周期轨道（Lunar Periodic Orbit）
description: 平面圆形限制性三体问题中绕月球运行的闭合轨道。与 L1 点的 Lyapunov 轨道类似，这类轨道在圆柱同构映射下可与不变流形置于同一相空间中分析。通过计算流形与月球周期轨道在 (x, y, γ) 空间中的交线，可找到以极小速度增量从 L1 流形转入月球附近长期驻留轨道的转移方案。
keywords: 月球周期轨道, Lunar Periodic Orbit, 周期轨道, 轨道转移, Halo轨道
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 月球周期轨道（Lunar Periodic Orbit）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 月球周期轨道详解 | 术语定义
  description: 平面圆形限制性三体问题中绕月球运行的闭合轨道。与 L1 点的 Lyapunov 轨道类似，这类轨道在圆柱同构映射下可与不变流形置于同一相空间中分析。通过计算流形与月球周期轨道在 (x, y, γ) 空间中的交线，可找到以极小速度增量从 L1 流形转入月球附近长期驻留轨道的转移方案。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 月球周期轨道详解 | 术语定义
  description: 平面圆形限制性三体问题中绕月球运行的闭合轨道。与 L1 点的 Lyapunov 轨道类似，这类轨道在圆柱同构映射下可与不变流形置于同一相空间中分析。通过计算流形与月球周期轨道在 (x, y, γ) 空间中的交线，可找到以极小速度增量从 L1 流形转入月球附近长期驻留轨道的转移方案。
  image: /logo.png
permalink: /glossary/orbits/lunar-periodic-orbit/
---

# 月球周期轨道（Lunar Periodic Orbit）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 本文地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

平面圆形限制性三体问题中绕月球运行的闭合轨道。与 L1 点的 Lyapunov 轨道类似，这类轨道在圆柱同构映射下可与不变流形置于同一相空间中分析。通过计算流形与月球周期轨道在 (x, y, γ) 空间中的交线，可找到以极小速度增量从 L1 流形转入月球附近长期驻留轨道的转移方案。

## 应用价值

在轨道设计阶段，可利用该轨道类型构建候选轨道池，为星座部署和任务轨道选择提供参考。在轨运行时，该轨道特性可用于轨道维持策略设计，降低推进剂消耗。在轨道转移规划中，其稳定流形结构可指导低能量转移走廊的搜索。

## 相关概念

- [运行轨道库（Operational Orbit Library）](/glossary/orbits/operational-orbit-library/)
- [月球自由返回轨道（Lunar Free-Return Orbit, LFO）](/glossary/orbits/lunar-free-return-orbit/)
- [临界轨道（Critical Orbit）](/glossary/orbits/critical-orbit/)
- [内部频率（Inner Frequencies）](/glossary/dynamics/inner-frequencies/)

## 参考文献

- Lunar capture trajectories and homoclinic connections through isomorphic mapping (Giancotti et al., 2012)
