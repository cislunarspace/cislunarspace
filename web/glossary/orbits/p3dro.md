---
title: 三倍周期DRO（Period-tripling DRO）
description: 从DRO分叉出的周期为DRO三倍的周期轨道。其附近的流形决定了黏附区的边界，低能捕获航天器需克服此稳定屏障。
keywords: 三倍周期DRO, Period-tripling DRO, P3DRO, 轨道, 转移, 周期轨道
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 三倍周期DRO（Period-tripling DRO）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 三倍周期DRO详解 | 术语定义
  description: 从DRO分叉出的周期为DRO三倍的周期轨道。其附近的流形决定了黏附区的边界，低能捕获航天器需克服此稳定屏障。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 三倍周期DRO详解 | 术语定义
  description: 从DRO分叉出的周期为DRO三倍的周期轨道。其附近的流形决定了黏附区的边界，低能捕获航天器需克服此稳定屏障。
  image: /logo.png
permalink: /glossary/orbits/p3dro/
---

# 三倍周期DRO（Period-tripling DRO）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 本文地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

从DRO分叉出的周期为DRO三倍的周期轨道。其附近的流形决定了黏附区的边界，低能捕获航天器需克服此稳定屏障。

## 应用价值

该概念在地月空间研究和工程实践中具有应用价值，有助于理解该领域的核心原理。

## 相关概念

- [月球自由返回轨道（Lunar Free-Return Orbit, LFO）](/glossary/orbits/lfo/)
- [弱稳定边界弹道转移（Weak Stability Boundary-like Ballistic Transfer, WSB-like）](/glossary/orbits/wsb-like/)
- [平面月球轨道（Planar Lunar Orbit, PLO）](/glossary/orbits/plo/)
- [低推力轨道转移（Low-thrust Orbit Transfer）](/glossary/orbits/low-thrust-orbit-transfer/)
## 参考文献

- Wang 等 - 2025 - Mechanism analysis of the DRO low-energy transfer problem An energy perspective。
