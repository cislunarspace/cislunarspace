---
title: 最小能量地月转移（Minimum Energy Trans-lunar Transfer）
description: 以最低能量消耗实现从地球到月球的越月转移，通过LL2点或晕轨道实现。
keywords: 最小能量地月转移, Minimum Energy Trans-lunar Transfer, 转移轨道, 周期轨道, 晕轨道
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 最小能量地月转移（Minimum Energy Trans-lunar Transfer）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 最小能量地月转移详解 | 术语定义
  description: 以最低能量消耗实现从地球到月球的越月转移，通过LL2点或晕轨道实现。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 最小能量地月转移详解 | 术语定义
  description: 以最低能量消耗实现从地球到月球的越月转移，通过LL2点或晕轨道实现。
  image: /logo.png
permalink: /glossary/orbits/minimum-energy-trans-lunar-transfer/
---

# 最小能量地月转移（Minimum Energy Trans-lunar Transfer）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 术语来源：Xu 等 - 2013 - On the construction of low-energy cislunar and trans-lunar transfers based on the libration points
>
> 本文地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

以最低能量消耗实现从地球到月球的越月转移，通过LL2点或晕轨道实现。

## 应用价值

最小能量地月转移是地月转移轨道设计和任务规划的核心要素。在多体动力学环境下，最小能量地月转移的选取直接影响转移时间、速度增量预算和任务安全性。工程中常通过数值方法搜索最优最小能量地月转移以平衡性能和风险。

## 相关概念

- [低能月球转移轨道（Low-Energy Lunar Transfer）](/glossary/orbits/low-energy-transfer/)
- [内外侧转移（Inner and Outer Transfer）](/glossary/orbits/low-energy-transfer/)
- [调相轨迹（Phasing Trajectory）](/glossary/orbits/phasing-trajectory/)
- [近火卫点距离（Periapsis Distance）](/glossary/orbits/periapsis-distance/)

## 参考文献

- Xu 等 - 2013 - On the construction of low-energy cislunar and trans-lunar transfers based on the libration points
