---
title: HL轨道（HL Orbit）
description: Halo轨道的简称，在本文中指围绕L1或L2点的非平面周期轨道。HL轨道相对于平动点增加了一维设计选择，可构造更多类型的穿越轨道和转移窗口。
keywords: HL轨道, HL Orbit, 轨道设计, 转移轨道, 平动点
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: HL轨道（HL Orbit）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: HL轨道详解 | 术语定义
  description: Halo轨道的简称，在本文中指围绕L1或L2点的非平面周期轨道。HL轨道相对于平动点增加了一维设计选择，可构造更多类型的穿越轨道和转移窗口。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: HL轨道详解 | 术语定义
  description: Halo轨道的简称，在本文中指围绕L1或L2点的非平面周期轨道。HL轨道相对于平动点增加了一维设计选择，可构造更多类型的穿越轨道和转移窗口。
  image: /logo.png
permalink: /glossary/orbits/hl-orbit/
---

# HL轨道（HL Orbit）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 本文来源：徐明 - 2010 - 地月低能转移的发生条件及轨迹构造
>
> 站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

Halo轨道的简称，在本文中指围绕L1或L2点的非平面周期轨道。HL轨道相对于平动点增加了一维设计选择，可构造更多类型的穿越轨道和转移窗口。

## 应用价值

该术语在地月空间任务中具有重要应用价值。在轨道设计阶段，工程师利用相关理论进行轨迹优化；在导航与轨道确定中，用于提升测量精度；在姿态控制与轨道保持任务中，确保航天器稳定运行。具体应用中，可结合任务需求进行参数优化和算法适配，提高任务成功率和资源利用效率。

## 相关概念

- [特征指数（Characteristic Exponents）](/glossary/dynamics/characteristic-exponents/)
- [捕获对接段（Capture Docking Phase）](/glossary/navigation/capture-docking-phase/)
- [月球借力转移（Lunar Flyby Transfer）](/glossary/orbits/lunar-flyby-transfer/)

## 参考文献

- 徐明 - 2010 - 地月低能转移的发生条件及轨迹构造
