---
title: 近共面构型（Near-Coplanar Configuration）
description: 两航天器的轨道面相对于月球轨道面接近平行的几何状态。在此构型下，仅用星间测距的LiAISON系统垂直于轨道面方向的观测信息严重不足，定轨精度大幅下降。
keywords: 近共面构型, Near-Coplanar Configuration, , 轨道设计, 轨道力学, 地月转移, 周期轨道, 晕轨道
author: 天疆说
date: 2026-07-31
...
wechatShare:
  ...
  image: /logo.png
og:
  title: 近共面构型（Near-Coplanar Configuration）详解 | 术语定义
  description: 两航天器的轨道面相对于月球轨道面接近平行的几何状态。在此构型下，仅用星间测距的LiAISON系统垂直于轨道面方向的观测信息严重不足，定轨精度大幅下降。
  image: /logo.png
  type: article
twitter:
  ...
  description: 两航天器的轨道面相对于月球轨道面接近平行的几何状态。在此构型下，仅用星间测距的LiAISON系统垂直于轨道面方向的观测信息严重不足，定轨精度大幅下降。
  image: /logo.png
permalink: /glossary/orbits/near-coplanar-configuration/
---

# 近共面构型（Near-Coplanar Configuration）
...
两航天器的轨道面相对于月球轨道面接近平行的几何状态。在此构型下，仅用星间测距的LiAISON系统垂直于轨道面方向的观测信息严重不足，定轨精度大幅下降。
