---
title: 晕轨道交会对接（Halo Orbit Rendezvous）
description: 在平动点晕轨道或近晕轨道上实现两个航天器相遇的技术。利用有效相位方法和相位脉冲机动可降低对接成本。
keywords: Halo Orbit Rendezvous, 周期轨道, 晕轨道, 晕轨道交会对接, 转移轨道
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 晕轨道交会对接（Halo Orbit Rendezvous）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 晕轨道交会对接详解 | 术语定义
  description: 在平动点晕轨道或近晕轨道上实现两个航天器相遇的技术。利用有效相位方法和相位脉冲机动可降低对接成本。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 晕轨道交会对接详解 | 术语定义
  description: 在平动点晕轨道或近晕轨道上实现两个航天器相遇的技术。利用有效相位方法和相位脉冲机动可降低对接成本。
  image: /logo.png
permalink: /glossary/orbits/halo-orbit-rendezvous/
---

# 晕轨道交会对接（Halo Orbit Rendezvous）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

在平动点晕轨道或近晕轨道上实现两个航天器相遇的技术。利用有效相位方法和相位脉冲机动可降低对接成本。

## 应用价值

在轨道设计与转移任务中，该概念用于分析航天器运动特性和设计转移方案，对任务燃料预算和发射窗口有重要影响。

## 相关概念

- [低能转移（Low-Energy Transfer）](/glossary/orbits/low-energy-transfer/)
- [准卫星轨道（Quasi-Satellite Orbit）](/glossary/orbits/quasi-satellite-orbit/)
- [混合轨道（Hybrid Trajectory）](/glossary/orbits/hybrid-trajectory/)
- [连续转移轨迹（Continuous Transfer Trajectory）](/glossary/orbits/continuous-transfer-trajectory/)

## 参考文献

- Perozzi和Ferraz-Mello - 2010 - Space manifold dynamics
