---
title: 径向振幅（Radial Amplitude）
description: Halo轨道在旋转坐标系中沿x轴（大天体到小天体连线方向）的最大偏移量。与面外振幅 A_z 和平面振幅 A_y 不同，A_x 反映轨道在两个主天体连线方向上的伸展范围。Richardson三阶近似解析解中，A_x 由 A_z 唯一确定，三者之间的约束关系是Halo轨道设计的基本约束。
keywords: 径向振幅, Radial Amplitude, A_x, 轨道, 轨道设计, 转移轨道
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 径向振幅（Radial Amplitude）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 径向振幅详解 | 术语定义
  description: Halo轨道在旋转坐标系中沿x轴（大天体到小天体连线方向）的最大偏移量。与面外振幅 A_z 和平面振幅 A_y 不同，A_x 反映轨道在两个主天体连线方向上的伸展范围。Richardson三阶近似解析解中，A_x 由 A_z 唯一确定，三者之间的约束关系是Halo轨道设计的基本约束。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 径向振幅详解 | 术语定义
  description: Halo轨道在旋转坐标系中沿x轴（大天体到小天体连线方向）的最大偏移量。与面外振幅 A_z 和平面振幅 A_y 不同，A_x 反映轨道在两个主天体连线方向上的伸展范围。Richardson三阶近似解析解中，A_x 由 A_z 唯一确定，三者之间的约束关系是Halo轨道设计的基本约束。
  image: /logo.png
permalink: /glossary/orbits/radial-amplitude/
---

# 径向振幅（Radial Amplitude）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
>本站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

Halo轨道在旋转坐标系中沿x轴（大天体到小天体连线方向）的最大偏移量。与面外振幅 A_z 和平面振幅 A_y 不同，A_x 反映轨道在两个主天体连线方向上的伸展范围。Richardson三阶近似解析解中，A_x 由 A_z 唯一确定，三者之间的约束关系是Halo轨道设计的基本约束。

## 应用价值

径向振幅是地月空间轨道设计中的重要参考轨型，可用于目标轨道选择和转移方案设计。
在实际任务中，基于径向振幅的轨道设计能够满足特定的任务约束，如通信仰角、覆盖范围和燃料最优等要求。
利用径向振幅的特性，可以优化转移轨道设计，实现从地球到月球的高效转移。
径向振幅为月球探测任务提供了多样化的轨道选择方案，有助于任务的灵活规划。

## 相关概念

- [月心段（Selenocentric Segment）](/glossary/orbits/selenocentric-segment/)
- [垂直振幅（Vertical Amplitude）](/glossary/orbits/vertical-amplitude/)
- [z方向运动振幅（z-direction Motion Amplitude）](/glossary/orbits/z-direction-motion-amplitude/)

## 参考文献

- Howell et al. 1997, Application of Dynamical Systems Theory to Trajectory Design for a Libration Point Mission
