...
最重要的一条 NRHO 是与月球会合周期成 9:2 共振的 L2 南族成员：每 2 个会合月（≈29.53 天）运行 9 圈，周期约 6.556 天；CR3BP 精确参数为近月点半径 3153 km、远月点半径 71230 km、雅可比常数 3.04719（Spreen 2021 附录 C），工程文献约作 6.5 天、3200/70000 km（Capannolo 2023；注意均为月心半径而非高度）。
...
- **Gateway 与 CAPSTONE**：L2 南族 9:2 会合共振 NRHO 是 Gateway 空间站的标称轨道（Zimovan-Spreen 2022）；CAPSTONE 于 2022 年进入该轨道并在轨运行，验证导航与轨道保持（晋守聪 2025、Qiao 2025）。
- **月球导航星座**：以 L1/L2 南北族 NRHO（按近月点半径 2000~16000 km 参数化）构建月球全球定位星座，最优构型为四轨 16 星，极区 GDOP 优于同规模晕轨道星座（晋守聪 2025）。共振导航星座研究中，L2 北/南族 NRHO（记 L2NH/L2SH，族周期约 1.37~3.42 个时间单位）在近地区、月球区、综合区三个目标区的最优构型 Rank 1/2 中全部出现；综合区最优为 L2NH-L2SH 与 L4/L5 垂直轨道的组合（He 2025）。
- **伴飞与编队**：Gateway 周围的准周期不变环面（QPT）可作为 Orion 伴飞的天然有界区域，以模型预测控制在环面上做相位重构（Capannolo 2023）。

## 到达与离开

- **NRHOI（入轨机动）**：进入 NRHO 的入轨机动。对比两种方案（Kikuchi 2024）：间接转移（IDT）的 NRHOI 在真近点角 160° 处施加，含大量非速度方向分量，约 240 m/s；近月点交会法（PRM）在近月点沿速度方向施加，仅约 55 m/s；PRM 先经近月点制动进入长椭圆绕月轨道等待约两周，待轨道面与 NRHO 对齐后一次入轨并直接与 Gateway 交会。
- **地球↔NRHO**：IDT 约 5.8 天/593 m/s（含 TLI），PRM 约 18.3 天/545 m/s，WSB 转移 >100 天但仅 60~100 m/s（Kikuchi 2024）；sGTO↔南族 L2 NRHO 低推力转移把稳定流形当长滑行弧，93 天/1541 m/s 量级（Singh 2021）。
- **NRHO↔DRO**：双脉冲转移利用对称性简化搜索，经近月飞掠与 Lyapunov 样轨道可压到 184~248 m/s（Wang 2021）；借邻域分岔族的流形约 20 天、200 m/s 量级且全程避食（Zimovan-Spreen 2022）。
- **Gateway↔他处**：高保真星历模型下 Gateway（9:2 NRHO）↔低月轨约 36 天、↔LEO 约 144~153 天的双向低推力转移（Pozzi 2025）。
- **低能耗调相（Liu 2025）**：NRHO 缺乏良好定义的不变流形，改用 Cauchy-Green 张量最大拉伸方向构造逃逸流/接近流并求交，实现追踪飞行器与目标站的相位匹配，比脉冲调相显著省燃料。
...
| L2NH / L2SH | L2 北 / 南族 NRHO（共振导航星座语境） | He 2025 |
...
- Pozzi et al., 2025, Gateway↔LLO/LEO 双向低推力转移相关研究
- 晋守聪 等, 2025, 基于 NRHO 的月球全球定位系统星座研究
