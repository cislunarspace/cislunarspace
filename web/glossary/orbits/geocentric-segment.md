---
title: 地心段（Geocentric Segment）
description: 地月转移轨道中从地球轨道出发、在EML1平动点与月心段衔接的地心动力学飞行段。
keywords: 地心段, Geocentric Segment, 转移轨道, 周期轨道, 晕轨道
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 地心段（Geocentric Segment）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 地心段详解 | 术语定义
  description: 地月转移轨道中从地球轨道出发、在EML1平动点与月心段衔接的地心动力学飞行段。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 地心段详解 | 术语定义
  description: 地月转移轨道中从地球轨道出发、在EML1平动点与月心段衔接的地心动力学飞行段。
  image: /logo.png
permalink: /glossary/orbits/geocentric-segment/
---

# 地心段（Geocentric Segment）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 术语来源：Minimum-fuel low-thrust trajectories to the moon (Yoon和Petukhov, 2023)
>
> 本文地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

地月转移轨道中从地球轨道出发、在EML1平动点与月心段衔接的地心动力学飞行段。

## 应用价值

地心段在地月空间动力学分析和任务轨道设计中具有应用价值。正确理解和应用地心段有助于优化轨道设计、控制策略和任务规划，是地月空间任务工程师需要掌握的基础知识。

## 相关概念

- [低能月球转移轨道（Low-Energy Lunar Transfer）](/glossary/orbits/low-energy-lunar-transfer/)
- [内外侧转移（Inner and Outer Transfer）](/glossary/orbits/inner-and-outer-transfer/)
- [调相轨迹（Phasing Trajectory）](/glossary/orbits/phasing-trajectory/)
- [近火卫点距离（Periapsis Distance）](/glossary/orbits/periapsis-distance/)

## 参考文献

- Minimum-fuel low-thrust trajectories to the moon (Yoon和Petukhov, 2023)
