---
title: 独热机动分类器（One-Hot Maneuver Classifier）
description: 基于估计控制积分与流形几何关系的多分类器，利用U-OCBE准确的事件重建能力。
keywords: 定位, 导航, 轨道确定, 星座
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 独热机动分类器（One-Hot Maneuver Classifier）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 独热机动分类器详解 | 术语定义
  description: 基于估计控制积分与流形几何关系的多分类器，利用U-OCBE准确的事件重建能力。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 独热机动分类器详解 | 术语定义
  description: 基于估计控制积分与流形几何关系的多分类器，利用U-OCBE准确的事件重建能力。
  image: /logo.png
permalink: /glossary/navigation/one-hot-maneuver-classifier/
---

# 独热机动分类器（One-Hot Maneuver Classifier）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 本文地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

基于估计控制积分与流形几何关系的多分类器，利用U-OCBE准确的事件重建能力。

## 应用价值

该方法在航天器轨道控制和状态估计中发挥关键作用。通过合理的控制策略设计或滤波算法选择，可以有效抑制扰动影响，提高航天器轨道保持精度和定轨收敛速度。

## 相关概念

- [位置精度因子（Position Dilution of Precision）](/glossary/navigation/position-dilution-of-precision/)
- [甚长基线干涉测量（Very Long Baseline Interferometry）](/glossary/navigation/very-long-baseline-interferometry/)
- [轨道预报（Orbit Prediction）](/glossary/navigation/orbit-prediction/)
- [初值点（Initial Epoch Point）](/glossary/navigation/initial-epoch-point/)

## 参考文献

- Autonomous Optical-Only Spacecraft-to-Spacecraft Absolute Tracking and Maneuver Classification in Cislunar Space
