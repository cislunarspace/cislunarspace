...
基于六维特征参数将观测状态转换为物理含义清晰的参数（周期、振幅等），在 Poincaré 截面图上建立参考轨道与截面坐标的对应关系，通过贝叶斯优化搜索使实际轨道与参考轨道的均方误差最小，从而实现轨道辨识。

### 关键指标

- **位置鲁棒性**：位置误差 < 100 km 时辨识结果波动小
- **速度鲁棒性**：速度误差 < 1 m/s 时辨识结果稳定
- **计算效率**：贝叶斯优化在 30 次评估内收敛
- **观测弧段要求**：长弧（约 1 个月）可收敛至参考轨道

### 系统组成

- **特征参数提取模块**：六维状态到六维特征参数的转换
- **参考轨道库**：CR3BP 标准轨道库（Halo、Lissajous 等）
- **贝叶斯优化引擎**：黑箱优化求解器
- **观测数据接口**：雷达/光学跟踪数据输入

## 应用价值

该方法为地月空间态势感知提供了一种不依赖长期数值积分的轨道辨识途径，在非合作目标、低精度观测条件下依然有效。可快速判定目标所处轨道族及其物理参数，服务于空间目标编目、碰撞预警和空间交通管理。
