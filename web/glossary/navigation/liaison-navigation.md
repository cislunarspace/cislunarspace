---
title: 联络导航（Liaison Navigation）
description: 由Hill和Born提出的平动点轨道卫星自主导航方法，通过卫星间的星间测距数据在无需地面支持的情况下自主确定轨道。2005年提出后被广泛应用于Halo轨道卫星的自主导航研究。该方法的核心是利用平动点附近非旋转对称的力场环境克服传统卫星导航中的秩亏问题。
keywords: 联络导航, Liaison Navigation, 自主导航, 轨道确定, 星间测距
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 联络导航（Liaison Navigation）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 联络导航详解 | 术语定义
  description: 由Hill和Born提出的平动点轨道卫星自主导航方法，通过卫星间的星间测距数据在无需地面支持的情况下自主确定轨道。2005年提出后被广泛应用于Halo轨道卫星的自主导航研究。该方法的核心是利用平动点附近非旋转对称的力场环境克服传统卫星导航中的秩亏问题。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 联络导航详解 | 术语定义
  description: 由Hill和Born提出的平动点轨道卫星自主导航方法，通过卫星间的星间测距数据在无需地面支持的情况下自主确定轨道。2005年提出后被广泛应用于Halo轨道卫星的自主导航研究。该方法的核心是利用平动点附近非旋转对称的力场环境克服传统卫星导航中的秩亏问题。
  image: /logo.png
permalink: /glossary/navigation/liaison-navigation/
---

# 联络导航（Liaison Navigation）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
>本站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

由Hill和Born提出的平动点轨道卫星自主导航方法，通过卫星间的星间测距数据在无需地面支持的情况下自主确定轨道。2005年提出后被广泛应用于Halo轨道卫星的自主导航研究。该方法的核心是利用平动点附近非旋转对称的力场环境克服传统卫星导航中的秩亏问题。

## 应用价值

自主导航技术使航天器能够在没有地面测控支持的情况下确定自身位置和速度，对地月空间探测任务的长期运行至关重要。

## 相关概念

- [秩亏（Deficient Rank）](/glossary/navigation/deficient-rank/)
- 多普勒测速（Doppler velocity measurement）
- 天文测角量（Astronometric Angle Measurement）
- 交会对接互操作标准（International Rendezvous System Interoperability Standard, IRSIS）

## 参考文献

- Liu et al. 2024, A novel autonomous navigation constellation in the Earth–Moon system
- Hill and Born 2008
