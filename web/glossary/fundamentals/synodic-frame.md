---
title: 会合坐标系（Synodic Frame）
description: 圆型限制性三体问题中常用的旋转参考坐标系。以两主天体的质心为原点，x轴由大质量主天体指向小质量主天体，z轴沿系统角动量方向，坐标系随两主天体连线同步旋转。在此坐标系下，两主天体位置固定不动，五个平动点成为定常平衡点，是研究平动点附近轨道动力学的标准框架。
keywords: 会合坐标系, Synodic Frame, 基础概念, 轨道力学, 坐标系统
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 会合坐标系（Synodic Frame）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 会合坐标系详解 | 术语定义
  description: 圆型限制性三体问题中常用的旋转参考坐标系。以两主天体的质心为原点，x轴由大质量主天体指向小质量主天体，z轴沿系统角动量方向，坐标系随两主天体连线同步旋转。在此坐标系下，两主天体位置固定不动，五个平动点成为定常平衡点，是研究平动点附近轨道动力学的标准框架。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 会合坐标系详解 | 术语定义
  description: 圆型限制性三体问题中常用的旋转参考坐标系。以两主天体的质心为原点，x轴由大质量主天体指向小质量主天体，z轴沿系统角动量方向，坐标系随两主天体连线同步旋转。在此坐标系下，两主天体位置固定不动，五个平动点成为定常平衡点，是研究平动点附近轨道动力学的标准框架。
  image: /logo.png
permalink: /glossary/fundamentals/synodic-frame/
---

# 会合坐标系（Synodic Frame）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
>本站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

圆型限制性三体问题中常用的旋转参考坐标系。以两主天体的质心为原点，x轴由大质量主天体指向小质量主天体，z轴沿系统角动量方向，坐标系随两主天体连线同步旋转。在此坐标系下，两主天体位置固定不动，五个平动点成为定常平衡点，是研究平动点附近轨道动力学的标准框架。

## 应用价值

会合坐标系是地月空间研究的基础概念，为理解更复杂的航天器动力学问题提供了理论支撑。
在实际工程应用中，会合坐标系的准确计算和分析对于任务成功至关重要。
掌握会合坐标系的概念有助于工程师进行轨道设计和任务规划。
会合坐标系为航天器轨道力学研究提供了基本框架，是进一步学习高级概念的基础。

## 相关概念

- [Bang-off-Bang推力剖面（Bang-off-Bang Thrust Profile）](/glossary/fundamentals/bang-off-bang-thrust-profile/)
- 春分点（Vernal Equinox）
- [L2-L1同伦法（L2-L1 Homotopy）](/glossary/fundamentals/l2-l1-homotopy/)

## 参考文献

- 关于探测器定点在共线平动点附近的控制问题
