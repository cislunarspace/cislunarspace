---
title: 伪测量值（Pseudo-measurement）
description: 将非直接测量的信息（如先验相对轨道）作为约束条件引入滤波器的虚拟测量量。其设计矩阵为单位矩阵，通过调节噪声权重控制约束强度。
keywords: 伪测量值, Pseudo-measurement, 轨道力学, 航天动力学, 数值方法
author: 天疆说
date: 2026-07-31
...
twitter:
  ...
  description: 将非直接测量的信息（如先验相对轨道）作为约束条件引入滤波器的虚拟测量量。其设计矩阵为单位矩阵，通过调节噪声权重控制约束强度。
  image: /logo.png
permalink: /glossary/fundamentals/pseudo-measurement/
---

# 伪测量值（Pseudo-measurement）
...
将非直接测量的信息（如先验相对轨道）作为约束条件引入滤波器的虚拟测量量。其设计矩阵为单位矩阵，通过调节噪声权重控制约束强度。
