...
吻切轨道根数（osculating orbital elements，又称 osculating elements、吻切根数）是摄动轨道运动中每一时刻对应的瞬时开普勒轨道根数。"Osculate" 源自拉丁文 "osculari"（接吻），指该瞬时开普勒椭圆与实际摄动轨迹在当前位置"相切"（Vallado 2022）。严格地说：若在时刻 $t$ 突然移除所有摄动力，航天器此后将以当前的位置 $\vec{r}$ 和速度 $\vec{v}$ 沿一个开普勒椭圆运动——这个椭圆就是该瞬时的吻切椭圆，其六个轨道根数即为吻切根数。
