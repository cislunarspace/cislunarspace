---
title: 地球扁率（Earth Oblateness）
description: 地球因自转导致赤道略微隆起、偏离完美球形的几何特征，通常用带谐系数（以J2为主）描述其非球形引力场。对三角平动点附近的相对运动，地球扁率引起的误差仅为毫米量级，可忽略不计。
keywords: 坐标系, 轨道, 物理, 天文
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 地球扁率（Earth Oblateness）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 地球扁率详解 | 术语定义
  description: 地球因自转导致赤道略微隆起、偏离完美球形的几何特征，通常用带谐系数（以J2为主）描述其非球形引力场。对三角平动点附近的相对运动，地球扁率引起的误差仅为毫米量级，可忽略不计。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 地球扁率详解 | 术语定义
  description: 地球因自转导致赤道略微隆起、偏离完美球形的几何特征，通常用带谐系数（以J2为主）描述其非球形引力场。对三角平动点附近的相对运动，地球扁率引起的误差仅为毫米量级，可忽略不计。
  image: /logo.png
permalink: /glossary/fundamentals/earth-oblateness/
---

# 地球扁率（Earth Oblateness）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 本文地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

地球因自转导致赤道略微隆起、偏离完美球形的几何特征，通常用带谐系数（以J2为主）描述其非球形引力场。对三角平动点附近的相对运动，地球扁率引起的误差仅为毫米量级，可忽略不计。

## 应用价值

该参考坐标系是进行轨道力学分析和任务设计的基础，在实际应用中需根据任务需求选择合适的坐标系。

## 相关概念

- [月心惯性坐标系（Moon-Centered Inertial Frame）](/glossary/fundamentals/moon-centered-inertial-frame/)
- NRLMSISE-00大气模型（NRLMSISE-00 Atmospheric Model）
- [会合参考系（Synodical Reference System）](/glossary/fundamentals/synodical-reference-system/)
- [线性周期系统（Linear Time-Periodic System）](/glossary/fundamentals/ltp/)

## 参考文献

- Catlin and McLaughlin, 2007
