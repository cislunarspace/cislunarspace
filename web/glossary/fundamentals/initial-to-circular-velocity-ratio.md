---
title: 初始-圆速度比（Initial-to-Circular Velocity Ratio）
description: 初始速度与当地圆轨道速度的比值，β=1 为圆轨道速度，β=√2 为逃逸速度。
keywords: Initial-to-Circular Velocity Ratio, β, 初始-圆速度比, 坐标系, 航天器, 轨道
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 初始-圆速度比（Initial-to-Circular Velocity Ratio）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 初始-圆速度比详解 | 术语定义
  description: 初始速度与当地圆轨道速度的比值，β=1 为圆轨道速度，β=√2 为逃逸速度。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 初始-圆速度比详解 | 术语定义
  description: 初始速度与当地圆轨道速度的比值，β=1 为圆轨道速度，β=√2 为逃逸速度。
  image: /logo.png
permalink: /glossary/fundamentals/initial-to-circular-velocity-ratio/
---

# 初始-圆速度比（Initial-to-Circular Velocity Ratio）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

初始速度与当地圆轨道速度的比值，β=1 为圆轨道速度，β=√2 为逃逸速度。

## 应用价值

在航天器动力学分析与设计中，该概念对理解航天器在地月空间中的运动特性和任务设计具有重要作用。

## 相关概念

- [亚轨道（Suborbital）](/glossary/fundamentals/suborbital/)
- [相对运动最优控制（Optimal Relative Motion Control）](/glossary/fundamentals/optimal-relative-motion-control/)
- [火箭分级（Rocket Staging）](/glossary/fundamentals/rocket-staging/)
- [叉乘矩阵（Cross-Product Matrix / Skew-Symmetric Matrix）](/glossary/fundamentals/cross-product-matrix-skew-symmetric-matrix/)

## 参考文献

- Topputo 2013