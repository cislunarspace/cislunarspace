---
title: 推力节流函数（Throttle Function）
description: 实际推力与最大推力的比值，取值范围 [0, 1]。最优节流函数在理想情况下为 bang-off-bang（取0或1），平滑后变为连续可微函数。
keywords: 推力节流函数, Throttle Function, 轨道力学, 天体测量, 坐标系
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 推力节流函数（Throttle Function）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 推力节流函数详解 | 术语定义
  description: 实际推力与最大推力的比值，取值范围 [0, 1]。最优节流函数在理想情况下为 bang-off-bang（取0或1），平滑后变为连续可微函数。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 推力节流函数详解 | 术语定义
  description: 实际推力与最大推力的比值，取值范围 [0, 1]。最优节流函数在理想情况下为 bang-off-bang（取0或1），平滑后变为连续可微函数。
  image: /logo.png
permalink: /glossary/fundamentals/throttle-function/
---

# 推力节流函数（Throttle Function）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 术语来源：Zhang et al. - 2025 - Smoothing technique for indirect low-thrust trajectory optimization in cislunar space
>
> 本文地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

实际推力与最大推力的比值，取值范围 [0, 1]。最优节流函数在理想情况下为 bang-off-bang（取0或1），平滑后变为连续可微函数。

## 应用价值

推力节流函数在地月空间科学与工程中有实际应用价值。相关研究支撑着地月空间任务的规划、实施和运营，是该领域知识体系的有机组成部分。

## 相关概念

- [误差超椭球拉伸方向（Stretching Direction of Error Hyper-Ellipsoid）](/glossary/fundamentals/stretching-direction-of-error-hyper-ellipsoid/)
- [贝叶斯更新（Bayesian Update）](/glossary/fundamentals/bayesian-update/)
- [实值稳定性指数（Real-Valued Stability Index）](/glossary/fundamentals/real-valued-stability-index/)
- [航迹角（Flight-Path Angle）](/glossary/fundamentals/flight-path-angle/)

## 参考文献

- Zhang et al. - 2025 - Smoothing technique for indirect low-thrust trajectory optimization in cislunar space
