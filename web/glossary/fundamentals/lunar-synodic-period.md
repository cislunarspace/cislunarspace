---
title: 月球会合周期（Lunar Synodic Period）
description: 地月日三者回到相同相对位置所需的时间，约为29.53天。它决定了月相的重复周期，也是设计共振DRO的基本时间尺度。2:1共振DRO的轨道周期约为14.77天，即半个会合周期。
keywords: 坐标系, 轨道, 物理, 天文
author: 天疆说
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: 月球会合周期（Lunar Synodic Period）
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 月球会合周期详解 | 术语定义
  description: 地月日三者回到相同相对位置所需的时间，约为29.53天。它决定了月相的重复周期，也是设计共振DRO的基本时间尺度。2:1共振DRO的轨道周期约为14.77天，即半个会合周期。
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 月球会合周期详解 | 术语定义
  description: 地月日三者回到相同相对位置所需的时间，约为29.53天。它决定了月相的重复周期，也是设计共振DRO的基本时间尺度。2:1共振DRO的轨道周期约为14.77天，即半个会合周期。
  image: /logo.png
permalink: /glossary/fundamentals/lunar-synodic-period/
---

# 月球会合周期（Lunar Synodic Period）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 本文地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

地月日三者回到相同相对位置所需的时间，约为29.53天。它决定了月相的重复周期，也是设计共振DRO的基本时间尺度。2:1共振DRO的轨道周期约为14.77天，即半个会合周期。

## 应用价值

该轨道设计方法在地月空间任务中广泛应用。通过优化轨道参数，可以在保证任务需求的前提下最大限度降低推进剂消耗，提高任务经济效益，是当前地月空间任务设计的重要工具。

## 相关概念

- [月心惯性坐标系（Moon-Centered Inertial Frame）](/glossary/fundamentals/moon-centered-inertial-frame/)
- NRLMSISE-00大气模型（NRLMSISE-00 Atmospheric Model）
- [会合参考系（Synodical Reference System）](/glossary/fundamentals/synodic-frame/)
- [线性周期系统（Linear Time-Periodic System）](/glossary/fundamentals/ltp/)

## 参考文献

- Welch et al., 2015, Mission Considerations for Transfers to a Distant Retrograde Orbit
