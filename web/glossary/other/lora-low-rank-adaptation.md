...
LoRA 的核心假设是预训练模型的参数更新矩阵具有低秩特性，即可以用两个低秩矩阵 $A \in \mathbb{R}^{d \times r}$ 和 $B \in \mathbb{R}^{r \times k}$（$r \ll \min(d, k)$）的乘积来有效近似。这一假设使得微调过程中的可训练参数量大幅减少，同时保持接近全量微调的性能。

### 技术特征

LoRA 的关键特性包括：（1）参数效率极高，通常仅需训练原始模型参数的 0.1%–3%；（2）推理时可将低秩矩阵合并回原始权重，不引入额外延迟；（3）支持为不同任务训练不同的低秩矩阵，实现高效的多任务切换。

### 实现方法

LoRA 的实现流程为：冻结预训练权重 $\Phi_0$，在目标层注入可训练的 $A$ 和 $B$ 矩阵（$A$ 高斯初始化，$B$ 零初始化），仅更新 $A$ 和 $B$ 的参数进行梯度下降训练，训练完成后合并权重 $\Phi = \Phi_0 + AB$。
...
LoRA 在航天器意图识别等空间智能任务中展现出强大的适配能力。通过 LoRA 微调大语言模型，可以在极低的计算和存储开销下将通用模型适配到航天领域特定任务，使在轨部署大模型成为可能。LoRA 还支持为不同任务维护独立的适配权重，便于构建多任务空间智能系统。
