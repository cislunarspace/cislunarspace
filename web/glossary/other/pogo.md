---
title: 纵向耦合振动(POGO)
description: 详细解析纵向耦合振动(POGO)的定义、产生机理、对液体火箭的影响规律及抑制技术，特别是针对多机并联可重复使用火箭的全剖面POGO抑制
keywords: POGO振动, 纵向耦合振动, 液体火箭, 多机并联, 可重复使用火箭, 推进剂输送系统, 稳定性控制
author: 天疆说
date: 2026-04-24
lastUpdated: 2026-04-24
wechatShare:
  title: 纵向耦合振动(POGO)
  desc: 地月空间研究前沿、术语定义与工具资源一站式学习。
  image: /logo.png
og:
  title: 纵向耦合振动(POGO)详解 | 液体火箭动力学
  description: 详细解析纵向耦合振动(POGO)的定义、产生机理、对液体火箭的影响规律及抑制技术
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: 纵向耦合振动(POGO)详解 | 液体火箭动力学
  description: 详细解析纵向耦合振动(POGO)的定义、产生机理、对液体火箭的影响规律及抑制技术
  image: /logo.png
permalink: /glossary/other/pogo/
---

# 纵向耦合振动（POGO）

> 本文作者：[天疆说](https://blog.csdn.net/qq_33254264)
>
> 本站地址：[https://cislunarspace.cn](https://cislunarspace.cn)

## 定义

纵向耦合振动（POGO）是液体运载火箭在飞行过程中出现的一种自激振动现象，表现为火箭沿纵轴方向的周期性结构振动与推进剂输送系统中的压力脉动之间形成闭环耦合。该现象最早在 1960 年代美国 Titan II 火箭的飞行中被系统识别，因其类似 Pogo 跳跳杆的纵向往复运动而得名。

POGO 振动一旦出现，会严重影响航天员的工作环境和飞行舒适度（超过 0.5 g 的振动峰值可导致视觉模糊），严重时可能导致箭体结构破坏或发动机工况异常，甚至造成任务失败。

## 核心要素

### 基本概念

POGO 振动的本质是火箭结构纵向模态与推进剂输送系统流体模态之间的正反馈耦合。当推力脉动与结构振动速度同相时，系统能量持续注入，振动幅值不断增长，形成自激振动。

### 技术特征

POGO 振动的频率由箭体结构纵向固有频率和推进剂输送系统流体振荡频率共同决定。关键参数包括蓄压器气体体积、管路长度和直径、发动机燃烧室响应特性。振动幅值超过 0.5 g 时可导致航天员视觉模糊，严重时可破坏结构。

### 实现方法

POGO 抑制分为主动和被动两类：被动方法通过在输送管路中安装蓄压器（可压缩气体腔）改变流体振荡频率，使其远离结构敏感模态；主动方法通过实时监测振动信号并调节阀门开度等参数实现自适应抑制，适用于多机并联可重复使用火箭的全剖面工况。

## 产生机理

POGO 振动的闭环耦合环节通常由以下四个环节构成：

1. **箭体结构振动**：火箭纵向结构模态被激发，引起箭体沿纵轴方向周期性地压缩和拉伸。
2. **推进剂输送系统压力脉动**：箭体的纵向振动通过力学耦合作用传导至推进剂贮箱和输送管路，产生推进剂供应流量的周期性扰动。
3. **发动机推力响应**：推进剂流量的周期性波动导致发动机燃烧室压力和推力产生同频脉动。
4. **反馈放大**：脉动推力通过箭体结构逆向反馈至推进剂输送系统，若相位匹配得当，将形成正反馈循环，使振动幅值持续增长。

四个环节之间的相位关系是决定 POGO 是否发生以及严重程度的核心因素。当推力脉动与结构振动速度同相时，系统能量持续注入，振动发散。

## 抑制方法

### 蓄压器（Accumulator）

在推进剂输送管路中安装蓄压器是最广泛使用的 POGO 被动抑制技术。蓄压器通过可压缩气体腔（通常为氦气）吸收推进剂的流量扰动，使输送系统的流体振荡频率偏离箭体结构敏感模态，从而切断耦合通道。对于多级运载火箭，需要根据各级输送系统的不同特性分别设置蓄压器参数。

### 主动抑制技术

对于现代可重复使用火箭（如多机并联、深度可调推力工况），传统被动蓄压器难以覆盖全飞行剖面的工况变化。主动 POGO 抑制技术通过实时监测振动信号并主动调节输送系统参数（如阀门开度、喷嘴压降特性），实现自适应振动抑制。

## 可重复使用火箭的特殊挑战

可重复使用火箭为 POGO 抑制带来了新的复杂性：

- **多机并联动力系统**：发动机集群之间的推力和流量差异会引入新的耦合模态。
- **深度可调节推力**：返回减速段的大幅变推力工况使输送系统的流体力学特性发生剧烈变化。
- **全剖面覆盖**：上升段与返回段的输送系统模式（如是否使用再循环、泵后压力水平等）显著不同，需要全剖面内的统一 POGO 抑制策略。

## 应用价值

POGO 抑制是液体运载火箭研制的核心技术之一。在可重复使用火箭时代，多机并联和深度变推力工况使 POGO 抑制面临新的挑战，主动抑制技术的突破将直接影响载人航天和重型运载火箭的飞行安全与可靠性。

## 相关概念

- [核热推进（Nuclear Thermal Propulsion）](/glossary/other/nuclear-thermal-propulsion/)

## 参考文献

- NASA SP-8055, Prevention of Coupled Structure-Propulsion Instability (POGO), 1970.
- Rubin S. Longitudinal instability of liquid rockets due to propulsion feedback [POGO](J). Journal of Spacecraft and Rockets, 1966.
- 2026年宇航领域科学问题和技术难题发布, 中国航天大会 (CSC2026), 2026.
