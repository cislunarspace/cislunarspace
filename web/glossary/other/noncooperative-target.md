...
### 基本概念
非合作目标的核心特征是不主动共享自身状态信息和意图，使得操作方必须通过外部观测手段获取目标的轨道状态和行为特征。非合作目标的存在是空间态势感知和在轨安全操作面临的主要挑战。

### 技术特征
非合作目标识别的关键技术特征包括：（1）依赖被动观测（光学、雷达）获取轨道数据，精度受观测条件限制；（2）意图推断需融合轨道动力学、搭载设备特征和环境条件等多源信息；（3）识别方法涵盖传统模糊推理、深度学习和大语言模型等多种技术路线。

### 实现方法
针对非合作目标的技术手段包括：高精度轨道确定与预报、相对运动模式分析（基于 CW 方程等动力学模型）、搭载设备类型识别（光学/雷达特征分析）、以及基于 LLM 的多源信息融合意图推理。
...
## 应用价值
非合作目标识别技术是空间安全和态势感知的核心能力。在空间站运行场景中，准确识别接近的非合作目标意图可为规避决策提供关键依据，保障在轨人员安全。随着在轨物体数量快速增长，非合作目标的自动识别和威胁评估能力对维护空间秩序和防止凯斯勒综合征具有战略意义。
