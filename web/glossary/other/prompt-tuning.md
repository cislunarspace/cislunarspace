...
twitter:
  ...
  description: 通过可学习的软提示token实现参数高效的大模型微调
  image: /logo.png
permalink: /glossary/other/prompt-tuning/
---
...
### 基本概念
提示调优的核心思想是通过在模型输入前添加可学习的连续向量（软提示），引导预训练模型适应下游任务，而不修改模型自身参数。软提示在连续向量空间中优化，可表达词表中不存在的语义信息，比人工设计的离散文本提示具有更强的表达能力。

### 技术特征
提示调优的关键特性包括：（1）参数效率极高，可训练参数通常不足模型总量的 1%；（2）软提示通过梯度下降自动优化，无需人工搜索提示模板；（3）P-tuning V2 在每个 Transformer 层中添加可训练嵌入，使性能在多个尺度上接近全量微调。

### 实现方法
P-tuning V2 的实现流程为：在输入序列前拼接 128 个可学习的软提示 token，在 LLM 每一层构建对应的可训练嵌入参数，冻结原始模型权重，仅训练软提示和逐层嵌入参数。训练完成后，模型参数为原始参数与新增参数的组合 $\Phi = \Phi_0 + \Delta\phi$。
...
## 应用价值
提示调优为大语言模型在航天领域的高效适配提供了轻量化方案。通过仅训练少量软提示参数即可将通用模型适配到航天器意图识别等特定任务，大幅降低微调成本。P-tuning V2 与思维链提示结合后，在 ChatGLM2-6B 上实现了 99.81% 的意图识别准确率，证明了该方法在空间安全应用中的有效性。
