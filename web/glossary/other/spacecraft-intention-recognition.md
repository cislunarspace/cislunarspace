...
航天器意图识别的核心是将威胁评估从"是否接近"提升到"为什么接近"的层面。通过融合轨道运动模式、搭载设备类型和环境条件等多源信息，推断目标航天器的运动意图（飞越、交会等）、操作意图（对接、抓取等）和任务意图（探测、攻击等）。

### 技术特征

意图识别的技术特征包括：（1）采用层次化意图分类体系（3 大类 23 种），覆盖从运动模式到任务目的的完整语义链；（2）支持基于轨道动力学、模糊推理、深度学习和大语言模型的多种识别方法；（3）大语言模型方法通过将多源信息转化为文本输入，利用推理能力进行综合判断。

### 实现方法

基于 LLM 的意图识别实现流程为：构建意图词汇语料库、设计提示元素和模板、通过仿真生成训练样本、使用 P-tuning V2 或 LoRA 微调 ChatGLM 等模型。测试了基础提示、指令提示和思维链提示三种策略，微调后模型准确率达 99.90%。
...
航天器意图识别是空间安全预警和态势感知的核心能力。在空间站运行场景中，自动识别接近目标的意图可为规避决策提供关键依据，保障在轨人员安全。该技术还可应用于空间态势感知中的行为分析、在轨服务任务规划以及空间攻防对抗中的威胁评估。
