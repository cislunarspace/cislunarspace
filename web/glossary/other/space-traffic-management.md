...
太空交通管控是一套涵盖空间物体感知、轨道预测、碰撞风险评估、规避决策与协同执行的综合管理体系。其核心目标是在日益拥挤的轨道环境中确保太空活动的安全性和可持续性，类比于地面的空中交通管理但技术复杂度远超后者。

### 技术特征

STM 的关键技术特征包括：（1）依赖地基雷达、天基光学和星间协同观测构建全域感知网络；（2）轨道预报精度受大气密度模型、太阳辐射压等摄动因素制约；（3）碰撞规避决策涉及概率评估、机动代价和国际协调等多维权衡。

### 实现方法

STM 的实施路径包括：建立空间态势感知网络编目在轨物体、发展高精度大气密度预报模型、构建碰撞概率评估与规避决策系统、以及推动"天路"等动态轨道分离走廊的规划与管理。
...
太空交通管控是维护空间安全和可持续利用的基础性能力。随着巨型星座部署和在轨物体数量激增，STM 对预防碰撞事件、避免凯斯勒综合征具有战略意义。在地月空间扩展活动中，STM 的技术框架将延伸至深空轨道管理，为月球基地运营和地月运输走廊规划提供安全保障。
