---
title: 地月空间入门指南
description: 地月空间知识库
keywords: 地月空间, 轨道力学, 三体问题, 探月工程
permalink: /
lastUpdated: 2026-08-19
footer: Apache Licensed | 湘ICP备2026006405号-1
wechatShare:
  title: 地月空间入门指南
  desc: 地月空间知识库
  image: /logo.png
---

# 地月空间入门指南

从近地轨道到三十八万公里之外的月球，航天器的飞行规律发生了根本变化。地球引力不再独占主导，月球引力与太阳引力相互交织，使空间轨道呈现出多体动力学的丰富形态。

无论是探月工程、载人登月，还是未来的深空物资中转，理解地月空间的力学环境、轨道族特性与工程工具都是基础。本站系统梳理地月空间的核心概念、计算方法与前沿动态，帮助读者建立清晰连贯的专业认知。

## 学习路径

建议按照由浅入深、循序渐进的三步路线阅读：

<div class="module-grid">
  <a href="/what-is-cislunarspace/" class="module-card module-card--what-is">
    <h3>1. 什么是地月空间</h3>
    <p>空间范围定义、环境特征与应用价值</p>
  </a>
  <a href="/cislunar-orbits/" class="module-card module-card--orbits">
    <h3>2. 地月空间轨道</h3>
    <p>NRHO、DRO、平动点轨道与转移通道</p>
  </a>
  <a href="/research-frontiers/" class="module-card module-card--research">
    <h3>3. 科学前沿与工程</h3>
    <p>研究方向、学术机构与重大任务</p>
  </a>
</div>

<div style="text-align: center; margin: 15px 0;">
  <a href="/what-is-cislunarspace/" class="start-learning-btn">
    从第一篇开始阅读 →
  </a>
</div>

## 查阅与实践工具

在阅读论文或开展工程设计时，可通过以下板块快速检索与验证：

<div class="module-grid">
  <a href="/glossary/" class="module-card module-card--glossary">
    <h3>术语词典</h3>
    <p>中英对照、概念辨析与数学定义</p>
  </a>
  <a href="/resources-tools/" class="module-card module-card--resources">
    <h3>数据与算法库</h3>
    <p>JPL星历、重力场模型与e2m2e代码</p>
  </a>
  <a href="/ai-chat" class="module-card module-card--ai-chat">
    <h3>AI 领域助手</h3>
    <p>基于全站知识库的交互式专业问答</p>
  </a>
</div>

## 开放协作

本站内容依托开源社区持续维护与更新，代码与文档托管于 GitHub 平台，欢迎同行学者与工程人员共同完善。
