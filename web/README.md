---
title: 地月空间入门指南
description: 地月空间研究一站式学习平台：科学概念、任务轨道、研究前沿、术语词典（含军事太空条令）、背景知识、资源工具与航天动态。
keywords: 地月空间, cislunar space, 入门指南, 术语词典, 军事太空条令, 太空优势, 轨道动力学, 研究前沿, 航天动态, 卫星仿真
permalink: /
heroImage: /logo.png
lastUpdated: 2026-04-29
footer: Apache Licensed | 湘ICP备2026006405号-1
wechatShare:
  title: 地月空间入门指南
  desc: 地月空间研究一站式学习平台，涵盖科学概念、术语词典、军事条令与航天动态。
  image: /logo.png
---

# 关于本站

> **本站致力于降低地月空间知识的学习门槛，所有内容均经过专业审核，力求准确、可靠、易于理解。**

本站按「概念—轨道—前沿—术语—工具—拓展」组织内容：可先读 [地月空间是什么](/what-is-cislunarspace/) 建立整体图景，再按需查阅 [轨道](/cislunar-orbits/)、[研究前沿](/research-frontiers/) 与 [术语词典](/glossary/)。词典已收录动力学模型、任务轨道、导航技术、月球矿物、机构组织及 [军事太空条令](/glossary/doctrine/space-superiority/) 等 19 类术语。[背景知识](/background/) 提供轨道力学、数学基础与控制理论的入门材料。动手环节可使用 [资源与工具](/resources-tools/) 与 [卫星轨道仿真](/satellite-simulation/)。[航天动态](/space-news/) 提供基于公开报道的业界摘录，[AI 问答](/ai-chat) 便于带着问题检索全站要点。

欢迎通过 [Gitee 仓库](https://gitee.com/cislunarspace/cislunarspace/) 或 [GitHub 仓库](https://github.com/cislunarspace/cislunarspace/) 提交词条、修订建议或其他贡献，与我们一同完善这座知识库。

## 主要模块

<div class="module-grid">
  <a href="/what-is-cislunarspace/" class="module-card module-card--what-is">
    <h3>地月空间是什么</h3>
    <p>定义、环境与战略背景</p>
  </a>
  <a href="/cislunar-orbits/" class="module-card module-card--orbits">
    <h3>地月空间轨道</h3>
    <p>任务轨道族与动力学直觉</p>
  </a>
  <a href="/research-frontiers/" class="module-card module-card--research">
    <h3>研究前沿</h3>
    <p>方向、机构、期刊与重大项目</p>
  </a>
  <a href="/glossary/" class="module-card module-card--glossary">
    <h3>术语词典</h3>
    <p>专业术语检索 · 含军事太空条令</p>
  </a>
  <a href="/background/" class="module-card module-card--background">
    <h3>背景知识</h3>
    <p>轨道力学、数学基础与控制理论</p>
  </a>
  <a href="/resources-tools/" class="module-card module-card--resources">
    <h3>资源与工具</h3>
    <p>数据集、开源代码与仿真平台</p>
  </a>
  <a href="/space-news/" class="module-card module-card--space-news">
    <h3>航天动态</h3>
    <p>政策、发射与任务摘录（中英）</p>
  </a>
  <a href="/satellite-simulation/" class="module-card module-card--simulation">
    <h3>卫星轨道仿真</h3>
    <p>在线轨道仿真教学平台</p>
  </a>
  <a href="/ai-chat" class="module-card module-card--ai-chat">
    <h3>AI 问答</h3>
    <p>AI 助手辅助学习</p>
  </a>
  <a href="/forum" class="module-card module-card--forum">
    <h3>论坛</h3>
    <p>交流讨论与问题反馈</p>
  </a>
</div>

<div style="text-align: center; margin: 40px 0;">
  <a href="/ai-chat" class="start-learning-btn">
    开始AI问答 →
  </a>
</div>

## 参与贡献

欢迎通过 [GitHub](https://github.com/cislunarspace) 或 [Gitee](https://gitee.com/cislunarspace) 提交词条、修订建议，与我们一同完善地月空间知识库。
