---
title: Amplitude Relation
description: The nonlinear constraint relationship between x and z amplitudes in halo orbits, determined by the Lindstedt-Poincaré method.
keywords: Amplitude Relation, orbits
author: Tianjiang Shuo
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: Amplitude Relation
  desc: Cislunar space research frontiers, term definitions, and tools resources.
  image: /logo.png
og:
  title: Amplitude Relation Explained | Term Definition
  description: The nonlinear constraint relationship between x and z amplitudes in halo orbits, determined by the Lindstedt-Poincaré method.
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: Amplitude Relation Explained | Term Definition
  description: The nonlinear constraint relationship between x and z amplitudes in halo orbits, determined by the Lindstedt-Poincaré method.
  image: /logo.png
permalink: /en/glossary/orbits/amplitude-relation/
---

# Amplitude Relation

> Author: [Tianjiang Shuo](https://blog.csdn.net/qq_33254264)
>
> Website: [https://cislunarspace.cn](https://cislunarspace.cn)

## Definition

The nonlinear constraint relationship between x and z amplitudes in halo orbits, determined by the Lindstedt-Poincaré method.

## Application Value

This concept provides fundamental support in cislunar space research and mission design. In practical applications, the appropriate analysis method should be selected based on specific mission scenarios and constraints.

## Related Concepts

- [Planar Distant Retrograde Orbit](/en/glossary/orbits/planar-distant-retrograde-orbit/)
- [Periselene](/en/glossary/orbits/periselene/)
- [Axial Resonant Orbit](/en/glossary/orbits/axial-resonant-orbit/)

## References

- Gómez 等 - 2001
