---
title: Orbital Stability Index
description: An index measuring the long-term stability of DRO under multi-body perturbations. Multiple cross-sections through the lunar center, perpendicular to the x-y pla
keywords: Orbital Stability Index
author: Tianjiang Shuo
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: Orbital Stability Index
  desc: Cislunar space research frontiers, term definitions, and tools resources.
  image: /logo.png
og:
  title: Orbital Stability Index Explained | Term Definition
  description: An index measuring the long-term stability of DRO under multi-body perturbations. Multiple cross-sections through the lunar center, perpendicular to the x-y pla
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: Orbital Stability Index Explained | Term Definition
  description: An index measuring the long-term stability of DRO under multi-body perturbations. Multiple cross-sections through the lunar center, perpendicular to the x-y pla
  image: /logo.png
permalink: /en/glossary/orbits/orbital-stability-index/
---

# Orbital Stability Index

> Author: [Tianjiang Shuo](https://blog.csdn.net/qq_33254264)
>
> Website: [https://cislunarspace.cn](https://cislunarspace.cn)

## Definition

An index measuring the long-term stability of DRO under multi-body perturbations. Multiple cross-sections through the lunar center, perpendicular to the x-y plane and equally spaced in phase angle, intersect the DRO trajectory; the average of the maximum distances between intersection points on each section quantifies the orbital boundedness and configuration preservation capability.

## Application Value

在周期轨道设计中，该指标用于评估轨道的有界性和长期稳定性，指导轨道维持策略的制定。

## Related Concepts

- [Synodic Resonant Periodic Orbit](/en/glossary/orbits/synodic-resonant-periodic-orbit/)
- [Halo Orbit Amplitude](/en/glossary/orbits/halo-orbit-amplitude/)
- [Near Rectilinear Halo Orbit](/en/glossary/orbits/near-rectilinear-halo-orbit/)
- [NRHO Transfer](/en/glossary/orbits/nrho-transfer/)

## References

- Zhou et al. 2024 - Design of circumlunar global positioning satellite constellation on DRO in the cislunar space
