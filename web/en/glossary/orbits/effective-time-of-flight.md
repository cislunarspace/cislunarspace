---
title: Effective Time of Flight
description: In low-energy transfer design, the actual flight time after subtracting the winding time spent near the Lyapunov orbit. Since most time on a manifold trajectory is consumed winding on and off the periodic orbit, a small maneuver can skip this phase, leaving the effective time of flight as a more practical engineering measure.
keywords: Effective Time of Flight
author: Tianjiang Shuo
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: Effective Time of Flight
  desc: Cislunar space research frontiers, term definitions, and tools resources.
  image: /logo.png
og:
  title: "Effective Time of Flight Explained | Term Definition"
  description: In low-energy transfer design, the actual flight time after subtracting the winding time spent near the Lyapunov orbit. Since most time on a manifold trajectory is consumed winding on and off the periodic orbit, a small maneuver can skip this phase, leaving the effective time of flight as a more practical engineering measure.
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: "Effective Time of Flight Explained | Term Definition"
  description: In low-energy transfer design, the actual flight time after subtracting the winding time spent near the Lyapunov orbit. Since most time on a manifold trajectory is consumed winding on and off the periodic orbit, a small maneuver can skip this phase, leaving the effective time of flight as a more practical engineering measure.
  image: /logo.png
permalink: /en/glossary/orbits/effective-time-of-flight/
---

# Effective Time of Flight
> Author: [Tianjiang Shuo](https://blog.csdn.net/qq_33254264)
>
> Website: [https://cislunarspace.cn](https://cislunarspace.cn)

## Definition

In low-energy transfer design, the actual flight time after subtracting the winding time spent near the Lyapunov orbit. Since most time on a manifold trajectory is consumed winding on and off the periodic orbit, a small maneuver can skip this phase, leaving the effective time of flight as a more practical engineering measure.

## Application Value

The Effective Time of Flight plays a significant role in cislunar space mission design, analysis, and control. In orbital design, it can be leveraged for transfer trajectory optimization; in navigation and control, it improves mission execution precision and reliability; in system analysis, it facilitates deeper understanding of complex multi-body dynamical behavior, guiding mission planning and risk assessment.


## Related Concepts

- [Football Orbit](/en/glossary/orbits/football-orbit/)
- [Orbit Chain](/en/glossary/orbits/orbit-chain/)
- [Prograde Polar Orbit](/en/glossary/orbits/prograde-polar-orbit/)
- [偏心倾角矢量分离（Eccentricity/Inclination Vector Separation）](/en/glossary/orbits/eccentricity-inclination-vector-separation/)


## References

- Fantino et al., 2010