---
title: Guidance, Navigation and Control
description: Collective term for spacecraft guidance, navigation and control systems
keywords: Guidance, Navigation and Control, GNC
author: Tianjiang Shuo
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: Guidance, Navigation and Control
  desc: Cislunar space research frontiers, term definitions, and tools resources.
  image: /logo.png
og:
  title: "Guidance, Navigation and Control Explained | Term Definition"
  description: Collective term for spacecraft guidance, navigation and control systems
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: "Guidance, Navigation and Control Explained | Term Definition"
  description: Collective term for spacecraft guidance, navigation and control systems
  image: /logo.png
permalink: /en/glossary/other/guidance-navigation-and-control/
---

# Guidance, Navigation and Control

> Author: [Tianjiang Shuo](https://blog.csdn.net/qq_33254264)
>
> Website: [https://cislunarspace.cn](https://cislunarspace.cn)

## Definition

Collective term for spacecraft guidance, navigation and control systems

## Application Value

GNC systems are critical for all spacecraft operations, providing autonomous or ground-assisted capabilities for trajectory guidance, state estimation, and attitude control throughout mission lifecycles.

## Related Concepts

- [Multibody Software](/en/glossary/other/multibody-software/)
- [Control Parametrization](/en/glossary/dynamics/control-parametrization/)
- [Coplanar Orbital Plane Constraint](/en/glossary/orbits/coplanar-orbital-plane-constraint/)
- [Thruster Modulator](/en/glossary/dynamics/thruster-modulator/)


## References

- Rendezvous in cislunar halo orbits: Hardware-in-the-loop simulation with coupled orbit and attitude dynamics
