---
title: Atmospheric Drag Perturbation
description: The orbital perturbation force caused by atmospheric drag on a spacecraft in near-Earth orbit. Although most of the Earth-Moon transfer orbit lies outside the atmosphere, the near-Earth segment near t
keywords: Atmospheric Drag Perturbation, cislunar space, orbital mechanics
author: Tianjiang Shuo
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: Atmospheric Drag Perturbation
  desc: Cislunar space research frontiers, term definitions, and tools resources.
  image: /logo.png
og:
  title: Atmospheric Drag Perturbation Explained | Term Definition
  description: The orbital perturbation force caused by atmospheric drag on a spacecraft in near-Earth orbit. Although most of the Earth-Moon transfer orbit lies outside the atmosphere, the near-Earth segment near t
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: Atmospheric Drag Perturbation Explained | Term Definition
  description: The orbital perturbation force caused by atmospheric drag on a spacecraft in near-Earth orbit. Although most of the Earth-Moon transfer orbit lies outside the atmosphere, the near-Earth segment near t
  image: /logo.png
permalink: /en/glossary/dynamics/atmospheric-drag-perturbation/
---

# Atmospheric Drag Perturbation

> Author: [Tianjiang Shuo](https://blog.csdn.net/qq_33254264)
>
> Website: [https://cislunarspace.cn](https://cislunarspace.cn)

## Definition

The orbital perturbation force caused by atmospheric drag on a spacecraft in near-Earth orbit. Although most of the Earth-Moon transfer orbit lies outside the atmosphere, the near-Earth segment near the insertion point is still affected. The paper employs an exponential atmosphere model in the precise force model to compute this perturbation, assuming atmospheric density decays exponentially with altitude. Atmospheric drag causes gradual semi-major axis reduction, cumulatively affecting the initial flight state in the near-Earth segment.

## Application Value

大气阻力摄动make近地orbit半长轴缓slow减小, In 地月transferorbit of 入轨点nearby仍有累积affect. In 精确orbitpredict中need考虑这一摄动因素.

## Related Concepts

- [Libration Point Orbit](/en/glossary/dynamics/libration-point-orbit/)
- [Invariant Manifold](/en/glossary/dynamics/invariant-manifold/)
- [Impulsive Maneuver](/en/glossary/dynamics/impulsive-maneuver/)
- [Halo Orbit](/en/glossary/dynamics/halo-orbit/)

## References

- 刘磊 等 - 2008 - 多约束条件下的地月转移轨道设计
- 肖峰 - 1989 - 球面天文学与天体力学基础