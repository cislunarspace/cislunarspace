---
title: Robust Unscented Kalman Filter
description: Unscented Kalman filter variant resilient to measurement malfunctions
keywords: Robust Unscented Kalman Filter, Robust Unscented Kalman Filter, RUKF
author: Tianjiang Shuo
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: Robust Unscented Kalman Filter
  desc: Cislunar space research frontiers, term definitions, and tools resources.
  image: /logo.png
og:
  title: Robust Unscented Kalman Filter Explained | Term Definition
  description: Unscented Kalman filter variant resilient to measurement malfunctions
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: Robust Unscented Kalman Filter Explained | Term Definition
  description: Unscented Kalman filter variant resilient to measurement malfunctions
  image: /logo.png
permalink: /en/glossary/dynamics/robust-unscented-kalman-filter/
---

# Robust Unscented Kalman Filter

> Author: [Tianjiang Shuo](https://blog.csdn.net/qq_33254264)
>
> Website: [https://cislunarspace.cn](https://cislunarspace.cn)

## Definition

Unscented Kalman filter variant resilient to measurement malfunctions

## Application Value

In orbital design, analysis, and control, dynamics models are used to predict spacecraft trajectories, with equations of motion solved numerically or analytically. This concept underpins critical mission capabilities including orbital maneuver design, orbit improvement, and formation flying.


## Related Concepts

- [Orbit Improvement](/en/glossary/dynamics/orbit-improvement/)
- [Cluster Aggregation](/en/glossary/dynamics/cluster-aggregation/)
- [Pseudospectral Method](/en/glossary/dynamics/pseudospectral-method/)


## References

- （暂无参考文献）
