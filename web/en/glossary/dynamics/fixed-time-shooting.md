---
title: Fixed-Time Shooting
description: A shooting method where flight time is fixed and does not change.
keywords: Fixed-Time Shooting
author: Tianjiang Shuo
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: Fixed-Time Shooting
  desc: Cislunar space research frontiers, term definitions, and tools resources.
  image: /logo.png
og:
  title: Fixed-Time Shooting Explained | Term Definition
  description: A shooting method where flight time is fixed and does not change.
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: Fixed-Time Shooting Explained | Term Definition
  description: A shooting method where flight time is fixed and does not change.
  image: /logo.png
permalink: /en/glossary/dynamics/fixed-time-shooting/
---

# Fixed-Time Shooting

> Author: [Tianjiang Shuo](https://blog.csdn.net/qq_33254264)

> Website: [https://cislunarspace.cn](https://cislunarspace.cn)

## Definition

A shooting method where flight time is fixed and does not change.

## Application Value

This concept is essential for understanding motion characteristics in orbital design and control, and plays a vital role in mission success.

## Related Concepts

- [Minimum Norm Solution](/en/glossary/dynamics/minimum-norm-solution/)
- [Rigid Body Dynamics](/en/glossary/dynamics/rigid-body-dynamics/)
- [Variable-Size Design Space, VSDS](/en/glossary/dynamics/variable-size-design-space-vsds/)
- [Analytical Gradient](/en/glossary/dynamics/analytical-gradient/)

## References

- Spreen 2021