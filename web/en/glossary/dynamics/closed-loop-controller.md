---
title: Closed-loop Controller
description: A controller that computes control acceleration based on real-time state deviation feedback.
keywords: Closed-loop Controller, dynamics
author: Tianjiang Shuo
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: Closed-loop Controller
  desc: Cislunar space research frontiers, term definitions, and tools resources.
  image: /logo.png
og:
  title: Closed-loop Controller Explained | Term Definition
  description: A controller that computes control acceleration based on real-time state deviation feedback.
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: Closed-loop Controller Explained | Term Definition
  description: A controller that computes control acceleration based on real-time state deviation feedback.
  image: /logo.png
permalink: /en/glossary/dynamics/closed-loop-controller/
---

# Closed-loop Controller

> Author: [Tianjiang Shuo](https://blog.csdn.net/qq_33254264)
>
> Website: [https://cislunarspace.cn](https://cislunarspace.cn)

## Definition

A controller that computes control acceleration based on real-time state deviation feedback.

## Application Value

该概念是分析地月空间动力学行为的基础工具。在轨道设计和任务分析中具有重要应用价值。

## Related Concepts

- [Hill Sphere Radius](/en/glossary/dynamics/hill-sphere-radius/)
- [Pseudospectral Convex Optimization](/en/glossary/dynamics/pseudospectral-convex-optimization/)
- [Poincaré Map Representation](/en/glossary/dynamics/poincar-map-representation/)
- [Minimum Norm Targeting](/en/glossary/dynamics/minimum-norm-targeting/)

## References

- Gao 等 - 2023 - Low-thrust station-keeping control for lunar near rectilinear halo orbits
