---
title: Extreme Terrain Mobility
description: Surface mobility capability over extreme terrain topographies and regolith properties on bodies with substantial gravity fields, including crater walls
keywords: Extreme Terrain Mobility, cislunar space, orbital mechanics, navigation, dynamics
author: Tianjiang Shuo
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: Extreme Terrain Mobility
  desc: Cislunar space research frontiers, term definitions, and tools resources.
  image: /logo.png
og:
  title: Extreme Terrain Mobility Explained | Term Definition
  description: Surface mobility capability over extreme terrain topographies and regolith properties on bodies with substantial gravity fields, including crater walls
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: Extreme Terrain Mobility Explained | Term Definition
  description: Surface mobility capability over extreme terrain topographies and regolith properties on bodies with substantial gravity fields, including crater walls
  image: /logo.png
permalink: /en/glossary/dynamics/extreme-terrain-mobility/
---

# Extreme Terrain Mobility

> Author: [Tianjiang Shuo](https://blog.csdn.net/qq_33254264)
>
> Website: [https://cislunarspace.cn](https://cislunarspace.cn)

## Definition

Surface mobility capability over extreme terrain topographies and regolith properties on bodies with substantial gravity fields, including crater walls, cliffs, sand dunes, gullies, canyons, cold traps, and fissures.

## Application Value

This concept has practical applications in cislunar space science and engineering. Related research supports the planning, implementation, and operations of cislunar missions, forming an integral part of the knowledge system in this field.

## Related Concepts

- [Multiple Segment Trajectory Design](/en/glossary/dynamics/multiple-segment-trajectory-design/)
- [Zero-Velocity Surface](/en/glossary/dynamics/zero-velocity-surface/)
- [Null Vector](/en/glossary/dynamics/null-vector/)
- [Circular Restricted Three-Body Problem](/en/glossary/dynamics/circular-restricted-three-body-problem/)

## References

- Starek 等 - 2016 - Spacecraft autonomy challenges for next-generation space missions
