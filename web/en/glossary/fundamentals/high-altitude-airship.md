---
title: High Altitude Airship (HAA)
description: A detailed analysis of High Altitude Airship (HAA) definitions, technical characteristics, typical programs, and current development status
keywords: High Altitude Airship, HAA, Stratospheric Airship, Long Endurance, Near-space Platform
author: CislunarSpace
date: 2026-04-29
lastUpdated: 2026-04-29
og:
  title: High Altitude Airship (HAA) | Stratospheric Platform
  description: A detailed analysis of High Altitude Airship (HAA) definitions, technical characteristics, typical programs, and current development status
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: High Altitude Airship (HAA) | Stratospheric Platform
  description: A detailed analysis of High Altitude Airship (HAA) definitions, technical characteristics, typical programs, and current development status
  image: /logo.png
permalink: /en/glossary/fundamentals/high-altitude-airship/
wechatShare:
  title: "Cislunar Space Guide | High Altitude Airship (HAA)"
  desc: "A detailed analysis of High Altitude Airship (HAA) definitions, technical characteristics, typical programs, and current development status"
  image: "/logo.png"
---

# High Altitude Airship (HAA)

> Author: [CislunarSpace](https://gitee.com/cislunarspace)
>
> Site: [https://cislunarspace.cn](https://cislunarspace.cn)

## Definition

A High Altitude Airship (HAA) is an unmanned aerostat designed to operate at stratospheric altitudes (typically 18-25 km) for long-endurance missions spanning weeks to months. As an important subclass of stratospheric airships, HAA focuses on near-space applications with emphasis on ultra-long endurance and regional station-keeping capability.

## Technical Specifications

| Specification | Value |
|:---|:---|
| Altitude | 18-25 km (typical), up to 30 km |
| Mission duration | 14-90 days |
| Payload capacity | 50-500 kg |
| Power | 1-10 kW (solar-powered) |
| Coverage radius | Hundreds of km |
| Energy source | Solar cells + Li-ion/Fuel cells |

## Key Programs

| Program | Country | Characteristics |
|:---|:---|:---|
| HiSentinel | USA | 50-80 km altitude, Army research |
| HALE-D | USA | DARPA program, endurance demonstration |
| Sceye HAPS | USA | Commercial platform, solar-powered |
| Project Loon | USA | Google X, communication coverage |
| Skyship-500 | UK | 500 m³ envelope |

## Key Technologies

1. **Lightweight high-strength envelope materials**: UV-resistant, low helium permeability
2. **Efficient solar systems**: Flexible GaAs/perovskite tandem cells
3. **Energy management**: Day-charging, night-discharging balance
4. **Autonomous control**: Autonomous decision-making in complex weather
5. **Buoyancy regulation**: Dynamic helium volume and ballast management

## Related Concepts

- [Stratospheric Airship](/en/glossary/fundamentals/stratospheric-airship/)
- [Near-space](/en/glossary/fundamentals/near-space/)
- [Regional Station-keeping Control](/en/glossary/dynamics/regional-station-keeping/)

## References

- Wang H, Chen W. HAA Overall Design[M]. Aviation Industry Press, 2025.
- DEFENSE A. High Altitude Airship Program Review[R]. DARPA, 2024.