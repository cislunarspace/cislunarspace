---
title: Universal Time, UT (UT)
description: Mean solar time on the Greenwich meridian, serving as the worldwide time reference.
keywords: Universal Time, UT, UT
author: Tianjiang Shuo
date: 2026-07-31
lastUpdated: 2026-07-31
wechatShare:
  title: Universal Time, UT (UT)
  desc: Cislunar space research frontiers, term definitions, and tools resources.
  image: /logo.png
og:
  title: Universal Time, UT (UT) Explained | Term Definition
  description: Mean solar time on the Greenwich meridian, serving as the worldwide time reference.
  image: /logo.png
  type: article
twitter:
  card: summary_large_image
  title: Universal Time, UT (UT) Explained | Term Definition
  description: Mean solar time on the Greenwich meridian, serving as the worldwide time reference.
  image: /logo.png
permalink: /en/glossary/fundamentals/universal-time-ut/
---

# Universal Time, UT

> Author: [Tianjiang Shuo](https://blog.csdn.net/qq_33254264)
>
> Website: [https://cislunarspace.cn](https://cislunarspace.cn)

## Definition

Mean solar time on the Greenwich meridian, serving as the worldwide time reference.

## Application Value

This concept is applicable in cislunar space mission design, orbit optimization, and trajectory planning for spacecraft transfers and maneuver strategies.

## Related Concepts

- [Control Regularization](/en/glossary/fundamentals/control-regularization/)
- [Cholesky Factorization](/en/glossary/fundamentals/cholesky-factorization/)
- [SPICE, SpiceyPy](/en/glossary/fundamentals/spice-spiceypy/)
- [Parabolic Orbit](/en/glossary/fundamentals/parabolic-orbit/)

## References

- 肖峰 - 1989 - 球面天文学与天体力学基础
