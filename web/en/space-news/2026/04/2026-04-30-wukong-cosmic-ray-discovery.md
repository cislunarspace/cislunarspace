---
layout: SpaceNewsArticle
title: "DAMPE Satellite Discovers Cosmic Ray Spectral 'Bump' Structure, Revealing Charge-Dependent Acceleration Limit"
description: "China's DAMPE (Wukong) dark matter particle detection satellite has directly confirmed for the first time, using 9 years of orbital observation data, that cosmic rays exhibit a 'bump' structure with particle counts sharply decreasing near a certain high-energy threshold, with the threshold value proportional to particle charge."
wechatShare:
  title: "DAMPE Satellite Discovers Cosmic Ray Spectral 'Bump' Structure, Revealing Charge-Dependent Acceleration Limit"
  desc: "China's DAMPE (Wukong) dark matter particle detection satellite has directly confirmed for the first time, using 9 years of orbital observation data, that cosmic rays exhibit a 'bump' structure with particle counts sharply decreasing near a certain high-energy threshold, with the threshold value proportional to particle charge."
  image: "./figures/2026-04-30-wukong-cosmic-ray-discovery/hero.png"
permalink: /en/space-news/2026/04/2026-04-30-wukong-cosmic-ray-discovery/
author: Tianjiang Shuo
date: 2026-04-30
lastUpdated: 2026-04-30
category: science
image: ./figures/2026-04-30-wukong-cosmic-ray-discovery/hero.png
---

# DAMPE Satellite Discovers Cosmic Ray Spectral 'Bump' Structure, Revealing Charge-Dependent Acceleration Limit

**Summary:** China's DAMPE (Wukong, meaning "Monkey King") dark matter particle detection satellite has been operating smoothly in orbit for over 10 years since its launch in late 2015. Using the satellite's first 9 years (2016-2024) of observation data, an international research team led by the Purple Mountain Observatory of the Chinese Academy of Sciences precisely measured the energy spectra of five cosmic ray species: protons, helium, carbon, oxygen, and iron. For the first time, they directly observed a consistent "bump" structure in all five particle species at high energies — a sharp decrease in particle counts at a specific energy level. More importantly, the research team discovered that the "limit value" is determined by the particle's electric charge rather than its mass, providing the first observational evidence for the charge-dependent law of cosmic ray acceleration energy limits. The results were published online in the international academic journal Nature on April 29, 2026.

![DAMPE satellite observation of cosmic ray spectral "bump" structure (Nature journal image)](/space-news/2026/04/figures/2026-04-30-wukong-cosmic-ray-discovery/hero.png)
*Credit: Nature / DAMPE Collaboration*

## Scientific Significance

The discovery reveals that there exists a "super particle accelerator" near Earth that can accelerate cosmic rays to extremely high energies. The fact that the acceleration limit is proportional to particle charge — rather than mass — provides critical new clues for understanding the mechanisms of cosmic ray acceleration in the universe.

The DAMPE satellite, also known as "Wukong," was launched in December 2015 and is China's first scientific satellite dedicated to dark matter particle detection. It has been operating reliably in orbit for over 10 years, making it one of China's longest-serving scientific satellites.

## Sources (original pages)

- [新华网：「悟空」号揭示宇宙射线加速关键机制，首次发现宇宙射线加速能量极限的电荷依赖规律（2026-04-30）](http://www.news.cn/2026-04/30/content_7024170.htm)
- [腾讯新闻：「悟空」号最新成果：地球附近存在「超级粒子加速器」（2026-04-30）](https://new.qq.com/rain/a/20260430A038NI00)
- [Nature: DAMPE Collaboration results (2026-04-29)](https://www.nature.com/articles/s41586-026-10472-0)
- [紫金山天文台：「悟空」号发布最新成果（2026-04-30）](https://www.pmo.cas.cn/)
