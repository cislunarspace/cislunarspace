...
function makeScan(): GlossaryScan {
  return {
    zh: {
      entries: [
        { slug: 'cr3bp', title: 'CR3BP', path: '/glossary/fundamentals/cr3bp/', category: fundamentals },
      ],
      missing: [
        { category: '基础概念', slug: 'lagrange-point', zhTitle: '拉格朗日点' },
      ],
    },
    en: {
      entries: [
        { slug: 'cr3bp', title: 'CR3BP', path: '/en/glossary/fundamentals/cr3bp/', category: fundamentals },
      ],
    },
  }
}
...
describe('buildChatIndexIntake', () => {
  it('builds glossary categories from taxonomy adapter ordering', () => {
    ...
    expect(index.en[0]).toEqual({
      ...
      entries: [
        ...
        { path: '/en/glossary/fundamentals/lagrange-point/', title: '拉格朗日点 (needs translation)' },
      ],
    })
  })
  ...
  it('appends non-glossary section categories from section taxonomy', () => {
    ...
    const enSection = index.en.find(c => c.category === 'Cislunar spacecraft orbits (mission trajectories)')

    expect(zhSection?.entries).toContainEqual({ path: '/cislunar-orbits/nrho/l1-nrho/', title: 'L1-NRHO' })
