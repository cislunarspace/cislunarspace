...
describe('buildRouteTable', () => {
  it('maps permalink to source file', () => {
    ...
    expect(table.get('/what-is-cislunarspace/')).toBe('what-is-cislunarspace/README.md');
  });
...
describe('extractLinks', () => {
  it('extracts markdown links with line numbers', () => {
    ...
    expect(links).toEqual([{ line: 2, text: 'link text', target: '/target/', kind: 'link' }]);
  });
...
describe('resolveLinks', () => {
  ...
  it('resolves relative directory-style links from README directory routes (issue #113)', () => {
    ...
    expect(ok[0]!.resolved).toBe('space-news/2026/04/2026-04-30-article.md');
...
describe('extractLinks > images', () => {
  it('extracts body images with line numbers', () => {
    ...
    const links = extractLinks(body);
    expect(links).toEqual([{ line: 2, text: 'alt text', target: '/img/photo.png', kind: 'image' }]);
  });
