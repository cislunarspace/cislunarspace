  ...
  if (frontmatter.layout !== 'SpaceNewsArticle') {
    return [
      issue(
        ...
        'wrong-layout',
        'error',
        `Layout must be SpaceNewsArticle, got "${frontmatter.layout}"`,
        'single-file',
        'layout',
      ),
    ];
...
export function detectDateSlugMismatch(file: MarkdownFile): FrontmatterIssue[] {
  ...
  if (!frontmatter.date) {
    return [
      issue(file.relPath, 'missing-date', 'error', 'Missing date field', 'single-file', 'date'),
    ];
  }
...
export function detectImageIssues(file: MarkdownFile): FrontmatterIssue[] {
  ...
  if (frontmatter.image === undefined) {
    return [
      issue(file.relPath, 'missing-image', 'error', 'Missing image field', 'single-file', 'image'),
    ];
  }
  ...
  if (rawImage === 'null') {
    return [
      issue(
        ...
        'null-image',
        'error',
        'Image value is the string "null"',
        'single-file',
        'image',
      ),
    ];
    ...
    if (m) {
      ...
      if (spaceAfterColon.length > 1) {
        ...
        return [
          issue(
            ...
            'image-leading-space',
            'error',
            `Image path has leading whitespace: "${spaceAfterColon}${rawValue}"`,
            'single-file',
            'image',
          ),
        ];
      ...
      if (rawValue !== rawValue.trimEnd()) {
        return [
          issue(
            file.relPath,
            'image-trailing-space',
            'error',
            `Image path has trailing whitespace: "${rawValue}"`,
            'single-file',
            'image',
          ),
        ];
...
export function formatTerminalOutput(
  ...
  const summaryParts = result.byRule.map((r) => `  ${r.ruleId}: ${r.count} [${r.severity}]`);
