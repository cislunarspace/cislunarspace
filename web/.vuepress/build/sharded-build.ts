...
function findAppJs(distDir: string): string | null {
  ...
  const files = readdirSync(path.join(distDir, 'assets')).filter((f) => /^app-.*\.js$/.test(f));
...
function mergeRouteTables(specs: ShardSpec[], distDir: string): void {
  ...
  const baseContent = readFileSync(destAppPath, 'utf8');
  const routesStart = baseContent.indexOf('Object.fromEntries(');
  if (routesStart === -1) {
    console.warn(
      '[sharded-build] WARN: could not find Object.fromEntries in app.js; skipping route merge',
    );
    return;
  }
  ...
  console.log(
    `[sharded-build] merged route table: ${allEntries.size} total routes into ${path.basename(destAppPath)}`,
  );
}
...
async function main(): Promise<void> {
  ...
  for (const s of specs) {
    ...
    console.log(
      `  shard ${s.index} (${s.role}): own=${own.length} exclude=${s.excludeMonths.length} months [${own.join(', ')}]`,
    );
  }
