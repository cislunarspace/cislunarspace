...
function wechatShareComplete(raw) {
  ...
  return /^\s+title:/m.test(raw) && /^\s+desc:/m.test(raw) && /^\s+image:/m.test(raw);
}
...
function frontmatterForNormalizer(raw) {
  ...
  const wechatShare =
    wsTitle != null || wsDesc != null || wsImage != null
      ? {
...
if (
  ...
  process.argv[1]?.endsWith('wechat-share-tool.mjs')
) {
