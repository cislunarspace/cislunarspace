...
import { detectGaps, detectKnowledgeBaseGaps, detectSpaceNewsGaps, detectRootPageGaps, matchesException, applyExceptions, extractSpaceNewsSlug, formatTerminalOutput, buildJsonReport, computeExitCode } from './check-bilingual-mirror'
import type { MarkdownFile } from '../utils/markdown-walker.ts'
import type { BilingualGap, GlossaryGapScanner, ExceptionRule, ContentFamily } from './check-bilingual-mirror-types.ts'
...
describe('detectGaps', () => {
  ...
  it('produces correct family summaries', () => {
    ...
    expect(result.byFamily).toEqual([
      { family: 'glossary', severity: 'error', gapCount: 3 },
    ])
  })
...
describe('detectKnowledgeBaseGaps', () => {
  it('detects missing English mirrors in knowledge-base sections', () => {
    ...
    expect(gaps[1]).toEqual(expect.objectContaining({
  ...
  it('ignores README files', () => {
    const files = [
      md('cislunar-orbits/README.md'),
      md('research-frontiers/README.md'),
    ]
...
describe('extractSpaceNewsSlug', () => {
  it('extracts slug from article path', () => {
    expect(extractSpaceNewsSlug('space-news/2026/03/2026-03-01-lunar-mission.md'))
      .toBe('2026-03-01-lunar-mission')
  })

  it('extracts slug from en article path', () => {
    expect(extractSpaceNewsSlug('en/space-news/2026/03/2026-03-01-lunar-mission.md'))
      .toBe('2026-03-01-lunar-mission')
  })
...
describe('matchesException', () => {
  it('matches exact path', () => {
    expect(matchesException('dialectic.md', [{ pattern: 'dialectic.md', reason: 'test' }])).toBe(true)
  })
  ...
  it('matches prefix with **', () => {
    ...
    expect(matchesException('research-frontiers/directions/_templates/sub/bar.md', rules)).toBe(true)
  ...
  it('returns false when no rules match', () => {
    ...
    expect(matchesException('glossary/fundamentals/a.md', [
      { pattern: 'dialectic.md', reason: 'test' },
    ])).toBe(false)
  })
})
...
describe('applyExceptions', () => {
  it('filters out gaps matching exception rules', () => {
    const gaps: BilingualGap[] = [
      { zhPath: 'glossary/fundamentals/a.md', expectedEnPath: 'en/glossary/fundamentals/a.md', family: 'glossary', severity: 'error', zhTitle: 'A' },
      { zhPath: 'dialectic.md', expectedEnPath: 'en/dialectic.md', family: 'root', severity: 'warning', zhTitle: '思辨' },
    ]
    const rules: ExceptionRule[] = [
      { pattern: 'dialectic.md', reason: 'zh-only' },
    ]
  ...
  it('returns empty matchedExceptions when no gaps match', () => {
    const gaps: BilingualGap[] = [
      { zhPath: 'a.md', expectedEnPath: 'en/a.md', family: 'glossary', severity: 'error', zhTitle: 'A' },
    ]
...
describe('buildJsonReport', () => {
  it('builds report with summary, gaps, and exceptions', () => {
    const gaps: BilingualGap[] = [
      { zhPath: 'a.md', expectedEnPath: 'en/a.md', family: 'glossary', severity: 'error', zhTitle: 'A' },
      { zhPath: 'b.md', expectedEnPath: 'en/b.md', family: 'space-news', severity: 'warning', zhTitle: 'B' },
    ]
...
describe('formatTerminalOutput', () => {
  it('returns summary and detail lines grouped by family', () => {
    const gaps: BilingualGap[] = [
      { zhPath: 'glossary/fundamentals/a.md', expectedEnPath: 'en/glossary/fundamentals/a.md', family: 'glossary', severity: 'error', zhTitle: 'Alpha' },
      { zhPath: 'glossary/dynamics/b.md', expectedEnPath: 'en/glossary/dynamics/b.md', family: 'glossary', severity: 'error', zhTitle: 'Beta' },
    ]
    const byFamily = [{ family: 'glossary' as ContentFamily, severity: 'error' as const, gapCount: 2 }]
  ...
  it('shows exceptions in output when provided', () => {
    const exceptions = [
      { pattern: 'dialectic.md', reason: 'zh-only special surface', matchedPaths: ['dialectic.md'] },
    ]
...
describe('computeExitCode', () => {
  ...
  it('returns 0 when only warnings exist and threshold is error', () => {
    const gaps: BilingualGap[] = [
      { zhPath: 'a.md', expectedEnPath: 'en/a.md', family: 'space-news', severity: 'warning', zhTitle: 'A' },
    ]
