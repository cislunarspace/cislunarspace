...
describe('buildSpaceNewsDirectoryView', () => {
  ...
  it('filters drafts from all homepage article collections', () => {
    ...
    expect(
      result.categorySections.flatMap((section) => section.items.map((article) => article.title)),
    ).toEqual(['Published']);
  });
  ...
  it('normalizes category labels and colors with fallbacks', () => {
    ...
    expect(
      ...
      },
      {
        title: 'Unknown',
        category: ['unknown-topic'],
        primaryCategory: 'unknown-topic',
        categoryLabel: 'unknown-topic',
  ...
  it('builds category sections localized per locale', () => {
    ...
    expect(
      zh.categorySections.map((section) => ({
        key: section.key,
        label: section.label,
        color: section.color,
      })),
    ...
    expect(
      en.categorySections.map((section) => ({
        key: section.key,
        label: section.label,
        color: section.color,
      })),
  ...
  it('builds homepage collections with existing limits and featured window', () => {
    ...
    expect(result.latestItems.map((article) => article.title)).toEqual([
      'A1',
      'A2',
      'A3',
      'A4',
    ...
    expect(
      result.categorySections
        .find((section) => section.key === 'artemis')
        ?.items.map((article) => article.title),
    ).toEqual(['A1', 'A2', 'A3']);
  });
  ...
  it('groups articles by year/month from relativePath, newest month first', () => {
    const articles = [
      makeArticle({
        ...
        path: '/may-13/',
        date: '2026-05-13',
        relativePath: 'space-news/2026/05/may-13.md',
        category: ['artemis'],
      }),
      makeArticle({
        title: 'May-01',
        path: '/may-01/',
        date: '2026-05-01',
        relativePath: 'space-news/2026/05/may-01.md',
        category: ['commercial'],
      }),
      makeArticle({
        title: 'Apr-15',
        path: '/apr-15/',
        date: '2026-04-15',
        relativePath: 'space-news/2026/04/apr-15.md',
        category: ['artemis'],
      }),
    ...
    expect(
      zh.monthGroups.map((group) => ({
        key: group.key,
        label: group.label,
        items: group.items.map((a) => a.title),
      })),
    ).toEqual([
  ...
  it('detects month from en-locale relativePath prefix', () => {
    const result = buildSpaceNewsDirectoryView({
      articles: [
        makeArticle({
          title: 'En article',
          path: '/en/space-news/2026/05/a/',
          date: '2026-05-12',
          relativePath: 'en/space-news/2026/05/a.md',
        }),
      ],
      locale: 'en',
      categoryMeta: testCategoryMeta,
    ...
    const result = buildSpaceNewsDirectoryView({
      articles: [
        makeArticle({
          ...
          path: '/d/',
          date: '2026-05-13',
          relativePath: 'space-news/2026/05/d.md',
          category: ['artemis'],
          draft: true,
        }),
        makeArticle({
          ...
          path: '/p/',
          date: '2026-05-12',
          relativePath: 'space-news/2026/05/p.md',
          category: ['commercial'],
        }),
      ],
      locale: 'zh',
