...
`;
    ...
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('nonexistent2025'));
    warnSpy.mockRestore();
  ...
  test('throws when ref.bib does not exist', () => {
    ...
    expect(() => generateBibliographyArtifacts(mdFiles, tmpDir, outDir)).toThrow(/not found/i);
  });
