...
function baseFileStem(): string {
  const path =
    (typeof window !== 'undefined' && window.location.pathname.replace(/\/$/, '')) || 'page';
...
function wrapTable(table: HTMLTableElement, index0: number) {
  ...
  toolbar.setAttribute('aria-label', t('表格导出', 'Table export'));
...
export function startTableEnhanceObserver() {
  ...
  const connect = () => {
    ...
    const target = document.querySelector('.vp-theme-container') || document.querySelector('main');
    if (!target) return;
    if (!target) return;
