...
export function kep2eci(a: number, e: number, i: number, raan: number, argp: number, nu: number) {
  ...
  return {
    pos: [Rxx * rx + Rxy * ry, Ryx * rx + Ryy * ry, Rzx * rx + Rzy * ry] as [
      number,
      number,
      number,
    ],
    vel: [Rxx * vx + Rxy * vy, Ryx * vx + Ryy * vy, Rzx * vx + Rzy * vy] as [
      number,
      number,
      number,
    ],
...
export function sunECI(epoch: Date) {
  ...
  return [
    AU * Math.cos(lam),
    AU * Math.cos(eps) * Math.sin(lam),
    AU * Math.sin(eps) * Math.sin(lam),
  ] as const;
}
