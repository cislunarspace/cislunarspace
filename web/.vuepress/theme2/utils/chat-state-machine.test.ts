...
describe('createChatStateMachine', () => {
  ...
  it('accumulates streaming content through callbacks', async () => {
    ...
    await sm.sendMessage('q', { locale: 'zh', t, createSession: () => session, saveCurrentChat: vi.fn() })
  ...
  it('removes the assistant placeholder on abort', async () => {
    ...
    const p = sm.sendMessage('q', { locale: 'zh', t, createSession: () => session, saveCurrentChat: vi.fn() })
    sm.abortRequest()
    await p
  ...
  it('surfaces network errors as assistant content', async () => {
    ...
    const session = createMockSession({
      route: vi.fn(async () => { throw new Error('offline') }),
    })
