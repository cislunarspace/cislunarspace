...
export function renderKatex(text: string): KatexRenderResult {
  ...
  result = result.replace(/(^|[^$])\$([^$\n]+)\$/g, (_, prefix, source) => prefix + pushPlaceholder(source, false))
...
export function renderInlineMarkdown(text: string, placeholders: string[]): string {
  ...
  html = html.replace(/(^|\s)(https?:\/\/[^\s<]+)/g, (_, prefix, href) => `${prefix}<a href="${escapeHtml(href)}" class="chat-link">${escapeHtml(href)}</a>`)
  // Auto-link bare internal paths
  html = html.replace(/(^|\s)(\/[A-Za-z0-9\-_/]+\/?(?:#[A-Za-z0-9\-_]+)?)/g, (_, prefix, href) => `${prefix}<a href="${escapeHtml(href)}" class="chat-link">${escapeHtml(href)}</a>`)
...
export function renderTableHtml(headerLine: string, bodyLines: string[], placeholders: string[]): string {
