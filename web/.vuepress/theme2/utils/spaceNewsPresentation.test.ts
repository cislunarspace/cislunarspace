...
describe('formatArticleDate', () => {
  ...
  it('en locale ignores the style flag (only zh distinguishes month width)', () => {
    expect(formatArticleDate('2026-05-13', 'en', 'short')).toBe(
      formatArticleDate('2026-05-13', 'en', 'long'),
    );
  });
});
...
describe('articleCardBackground', () => {
  it('uses the image url when present, with gradient fallback', () => {
    expect(
      ...
      background:
        'url(/img/x.png) center/cover no-repeat, linear-gradient(155deg, #0f172a 0%, #17203a 55%, #1e3a8a 130%)',
    });
  });
