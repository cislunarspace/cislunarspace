 ...
 */
function makeMarkdownFile(partial: { relPath?: string; frontmatter?: Record<string, unknown> }) {
...
describe('filesToArticles', () => {
  it('filters out README files', () => {
    const files = [
      makeMarkdownFile({
        relPath: 'space-news/2024/01/README.md',
  ...
  it('filters out draft files', () => {
    const files = [
      makeMarkdownFile({
        relPath: 'space-news/2024/01/draft.md',
        frontmatter: { title: 'Draft', draft: true },
      }),
      makeMarkdownFile({
        relPath: 'space-news/2024/01/published.md',
  ...
  it('passes through array category as-is (single and multi-entry)', () => {
    const files = [
      makeMarkdownFile({
        relPath: 'space-news/2024/01/a.md',
        frontmatter: { title: 'A', category: ['artemis'] },
      }),
      makeMarkdownFile({
        relPath: 'space-news/2024/01/b.md',
        frontmatter: { title: 'B', category: ['artemis', 'commercial'] },
      }),
    ];
    const result = filesToArticles(files, 'space-news/', '/space-news/');
